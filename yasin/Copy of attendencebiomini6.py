...
class NBioBSPWrapper:
    ...
    def verify_fingerprint(self, captured_template, stored_template):
        ...
        try:
            ...
            res = self.dll_matcher.UFM_Verify(
                self.hMatcher, 
                t1_buf, len(t1_bytes), 
                t2_buf, len(t2_bytes), 
                ctypes.byref(match)
    ...
    def start_enrollment(self):
        ...
        for idx, finger_idx in enumerate(selected_fingers):
            ...
            while template is None:
                # Check if user closed the window to stop the loop
                ...
                if template:
                    ...
                    self.status_text.append(f"✓ {finger_name} captured successfully!")
                    self.status_text.append("-----------------------------")
                else:
                    self.status_text.append(f"⚠ Low Quality or Failed. Retrying {finger_name}...")
                    self.status_text.append("Please lift finger and press harder...")
                    QApplication.processEvents()
                    # Small delay to let user lift finger
