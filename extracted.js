/* Script Block 2 */
        ...
        window.onerror = function(msg, url, lineNo, columnNo, error) {
            ...
            // Optionally alert the user in a non-intrusive way
            return false;
        };
...
/* Script Block 3 */
        ...
        const STORE_MONTHLY_DUES = 'monthlyDues';
        const STORE_EXAMS = 'exams';
        const STORE_SUBJECTS = 'subjects';
        const LOW_STOCK_THRESHOLD = 3;
        const NAV_CONFIG = [
            { text: 'ڈیش بورڈ', moduleId: 'dashboard' },
            { text: 'طلباء', moduleId: 'students' },
            { text: 'اساتذہ', moduleId: 'teachers' },
            { text: 'کلاسیں', moduleId: 'classes' },
            { text: 'فیس', moduleId: 'fees' },
            { text: 'فیس اسٹرکچر', moduleId: 'feeStructures' },
            { text: 'حاضری', moduleId: 'attendance' },
            { text: 'اسٹاف حاضری', moduleId: 'staffAttendance' },
            { text: 'نتائج', moduleId: 'results' },
            { text: 'ٹائم ٹیبل / ڈیوٹی روسٹر', moduleId: 'timetable' },
            { text: 'اخراجات', moduleId: 'expenses' },
            { text: 'رپورٹس', moduleId: 'reports' },
            { text: 'انوینٹری', moduleId: 'inventory' },
            { text: 'سیٹنگز', moduleId: 'settings' },
            { text: 'ڈیٹا ایکسپورٹ', moduleId: 'exportData' },
            { text: 'آئی ڈی اور سرٹیفیکٹ', moduleId: 'idCardGenerator' }
            /*
             Add any new menu items here only!
             */
        ];
        function openDatabase() {
            return new Promise((resolve, reject) => {
                ...
                request.onupgradeneeded = (event) => {
                    ...
                    if (!db.objectStoreNames.contains('staffAttendance')) {
                        const staffAttendanceStore = db.createObjectStore('staffAttendance', { keyPath: 'id', autoIncrement: true });
                        staffAttendanceStore.createIndex('date_staffId', ['date', 'staffId'], { unique: true });
                    }
                    ...
                    if (!db.objectStoreNames.contains('monthlyDues')) {
                        const duesStore = db.createObjectStore('monthlyDues', { keyPath: 'id', autoIncrement: true });
                        duesStore.createIndex('studentId', 'studentId', { unique: false });
                        duesStore.createIndex('monthYear', 'monthYear', { unique: false });
                        duesStore.createIndex('studentId_feeType_monthYear', ['studentId', 'feeType', 'monthYear'], { unique: true });
                    } else {
                        const duesStore = upgradeTransaction.objectStore('monthlyDues');
                        if (duesStore.indexNames.contains('student_fee_month')) {
                            duesStore.deleteIndex('student_fee_month');
                        }
                        if (!duesStore.indexNames.contains('studentId_feeType_monthYear')) {
                            duesStore.createIndex('studentId_feeType_monthYear', ['studentId', 'feeType', 'monthYear'], { unique: true });
                        }
                    }
                    ...
                    if (!db.objectStoreNames.contains(STORE_INVENTORY)) {
                        const inventoryStore = db.createObjectStore(STORE_INVENTORY, { keyPath: 'id', autoIncrement: true });
                        inventoryStore.createIndex('itemName', 'itemName', { unique: false });
                        inventoryStore.createIndex('itemCategory', 'itemCategory', { unique: false });
                        inventoryStore.createIndex('quantity', 'quantity', { unique: false });
                    } else {
                        const inventoryStore = upgradeTransaction.objectStore(STORE_INVENTORY);
                        if (!inventoryStore.indexNames.contains('itemName')) {
                            inventoryStore.createIndex('itemName', 'itemName', { unique: false });
                        }
                        if (!inventoryStore.indexNames.contains('itemCategory')) {
                            inventoryStore.createIndex('itemCategory', 'itemCategory', { unique: false });
                        }
                        if (!inventoryStore.indexNames.contains('quantity')) {
                            inventoryStore.createIndex('quantity', 'quantity', { unique: false });
                        }
                    }
        ...
        async function generateRecurringDues() {
            console.log("Checking for recurring dues generation...");
            const today = new Date();
            const currentMonth = today.getMonth();
            const currentYear = today.getFullYear();
            const currentMonthYear = `${currentYear}-${String(currentMonth + 1).padStart(2, '0')}`;
            const lastCheck = localStorage.getItem('duesGeneratedFor');
            if (lastCheck === currentMonthYear) {
                console.log(`Dues for ${currentMonthYear} already generated.`);
                return;
            }
            try {
                const allFeeStructures = await getAll('feeStructures');
                const recurringStructures = allFeeStructures.filter(fs =>
                    fs.feeType === 'Monthly Fee' || fs.feeType === 'Yearly Fee'
                );
                if (recurringStructures.length === 0) {
                    localStorage.setItem('duesGeneratedFor', currentMonthYear);
                    return;
                }
                const allStudents = await getAll(STORE_STUDENTS);
                const dueStore = await getObjectStore('monthlyDues', 'readwrite');
                let duesCreatedCount = 0;
                for (const student of allStudents) {
                    const classStructures = recurringStructures.filter(fs => fs.classId === student.classId);
                    for (const structure of classStructures) {
                        let dueMonthYear = null;
                        if (structure.feeType === 'Monthly Fee') {
                            dueMonthYear = currentMonthYear;
                        } else if (structure.feeType === 'Yearly Fee' && currentMonth === 0) {
                            dueMonthYear = `${currentYear}`;
                        }
                        if (dueMonthYear) {
                            const dueRecord = {
                                ...
                                monthYear: dueMonthYear,
                                status: 'due',
                                amountPaid: 0,
                                discount: 0
                            };
                            const index = dueStore.index('studentId_feeType_monthYear');
                            const checkReq = index.get([student.id, structure.feeType, dueMonthYear]);
                            const existingDue = await new Promise((resolve, reject) => {
                                checkReq.onsuccess = () => resolve(checkReq.result);
                                checkReq.onerror = (e) => reject(e.target.error);
                            });
                            if (!existingDue) {
                                await new Promise((resolve, reject) => {
                                    const addReq = dueStore.add(dueRecord);
                ...
                if (duesCreatedCount > 0) {
                    console.log(`${duesCreatedCount} new recurring dues generated for ${currentMonthYear}.`);
                }
                localStorage.setItem('duesGeneratedFor', currentMonthYear);
            } catch (error) {
                console.error("Failed to generate recurring dues:", error);
            }
        }
        ...
        function showModule(moduleId) {
            // Standard cleanup and active state management
            ...
            document.querySelectorAll('#main-nav-container button, .mobile-menu button').forEach(button => {
                button.classList.remove('active');
            });
            const activeButton = document.querySelector(`button[data-module-id="${moduleId}"]`);
            if (activeButton) {
                activeButton.classList.add('active');
            }

            // Table enhancement side effects (from line 5122 override)
            if (typeof initializedTables !== 'undefined') {
                ...
                if (typeof initializeTables === 'function') setTimeout(initializeTables, 500);
            }

            // Module specific logic
            if (moduleId === 'staffAttendance') {
                const dateInput = document.getElementById('staffAttendanceDate');
                if (dateInput) dateInput.value = new Date().toISOString().split('T')[0];
                if (typeof loadStaffForAttendance === 'function') loadStaffForAttendance();
            }
            if (moduleId === 'students' && typeof loadStudents === 'function') loadStudents();
            if (moduleId === 'teachers' && typeof loadTeachers === 'function') loadTeachers();
            if (moduleId === 'classes' && typeof loadClasses === 'function') loadClasses();
            if (moduleId === 'fees') { 
                if (typeof loadStudentsForFeeSelect === 'function') loadStudentsForFeeSelect(); 
                if (typeof loadStudentFeeDetails === 'function') loadStudentFeeDetails(); 
            }
            if (moduleId === 'attendance') { 
                if (typeof loadClassesForAttendance === 'function') loadClassesForAttendance(); 
                if (typeof loadStudentsForAttendance === 'function') loadStudentsForAttendance(); 
            }
            if (moduleId === 'results') {
                if (typeof createResultsModule === 'function') createResultsModule();
                setTimeout(() => {
                    if (typeof loadClassesForResults === 'function') loadClassesForResults();
                    if (typeof loadStudentsForResultCard === 'function') loadStudentsForResultCard();
                    if (typeof loadExamsForResultCard === 'function') loadExamsForResultCard();
                    if (typeof loadPastResults === 'function') loadPastResults();
                }, 100);
            }
            ...
            if (moduleId === 'feeStructures') { 
                if (typeof loadClassesForFeeStructureFilter === 'function') loadClassesForFeeStructureFilter(); 
                if (typeof loadFeeStructures === 'function') loadFeeStructures(); 
            }
            if (moduleId === 'timetable') { 
                if (typeof loadClassesForTimetable === 'function') loadClassesForTimetable(); 
                if (typeof loadTeachersForRoaster === 'function') loadTeachersForRoaster(); 
                if (typeof loadTimetable === 'function') loadTimetable(); 
            }
            if (moduleId === 'dashboard') {
                if (typeof loadDashboardStats === 'function') loadDashboardStats();
                if (typeof populateDashboardDateSelectors === 'function') populateDashboardDateSelectors();
                if (typeof toggleDashboardReportInputs === 'function') toggleDashboardReportInputs();
                if (typeof loadAdvancedFilterData === 'function') loadAdvancedFilterData();
            }
            if (moduleId === 'reports' && typeof loadClassesForReportFilter === 'function') loadClassesForReportFilter();
            if (moduleId === 'settings' && typeof loadPrintSettings === 'function') loadPrintSettings();
            if (moduleId === 'inventory') {
                if (typeof loadInventory === 'function') loadInventory();
                if (typeof checkLowStockItems === 'function') checkLowStockItems();
                if (typeof loadInventoryCategories === 'function') loadInventoryCategories();
            }
            
            // Export Data side effects (from line 6072 override)
            if (moduleId === 'exportData') {
                const storeSelect = document.getElementById('exportStoreSelect');
                const tableCont = document.getElementById('exportTableContainer');
                if (storeSelect) storeSelect.value = '';
                if (tableCont) tableCont.style.display = 'none';
            }

            // ID Card Generator side effects (from line 6788 override)
            if (moduleId === 'idCardGenerator') {
                if (typeof ensureQRCodeLibrary === 'function') {
                    ensureQRCodeLibrary().then(loaded => {
                        if (loaded) console.log("ID Card module shown, QR library ready.");
                    });
                }
                ...
                if (typeof loadStudentsForBulkCertificate === 'function') loadStudentsForBulkCertificate();
                if (typeof handleDataSourceChange === 'function') handleDataSourceChange();
            }

            if (typeof loadClassesForStudentForm === 'function') loadClassesForStudentForm();
            if (typeof loadTeachersForClassForm === 'function') loadTeachersForClassForm();
        }
        ...
        async function loadInventory() {
            try {
                ...
                const inventory = await getAll(STORE_INVENTORY);
                ...
                inventory.forEach(item => {
                    if (!searchTerm ||
                        ...
                        row.innerHTML = `
                            ...
                            <td>
                                <button onclick="editInventoryItem(${item.id})" class="edit">ترمیم</button>
                                <button onclick="deleteInventoryItem(${item.id})" class="delete">حذف</button>
                            </td>
                        `;
                    }
                });
        ...
        async function loadInventoryCategories() {
            try {
                const inventory = await getAll(STORE_INVENTORY);
                const categories = [...new Set(inventory.map(item => item.itemCategory))];
                const categoryInput = document.getElementById('itemCategory');
                categoryInput.setAttribute('list', 'inventoryCategories');
                const datalist = document.getElementById('inventoryCategories') ||
                ...
                if (!document.getElementById('inventoryCategories')) {
        ...
        async function editInventoryItem(id) {
            try {
                const item = await getById(STORE_INVENTORY, id);
                if (item) {
        ...
        async function deleteInventoryItem(id) {
            if (confirm('کیا آپ واقعی یہ آئٹم حذف کرنا چاہتے ہیں؟')) {
                try {
                    await deleteData(STORE_INVENTORY, id);
                    loadInventory();
                } catch (error) {
        ...
        document.getElementById('inventoryForm').addEventListener('submit', async function (e) {
            ...
            try {
                if (id) {
                    item.id = parseInt(id);
                    await updateData(STORE_INVENTORY, item);
                } else {
                    await addData(STORE_INVENTORY, item);
                    if (item.unitPrice > 0 && item.quantity > 0) {
                        const expenseAmount = item.quantity * item.unitPrice;
                        const expenseTransaction = {
                            date: item.purchaseDate || new Date().toISOString().split('T')[0],
                            recordType: 'expense',
                            category: 'انوینٹری خریداری',
                            amount: expenseAmount,
                            description: `خریداری: ${item.quantity} عدد ${item.itemName} @ ${item.unitPrice} فی عدد`,
                            paidTo: 'سپلائر'
                        };
                        await addData(STORE_EXPENSES, expenseTransaction);
                        loadDashboardStats();
                    }
                }
                resetInventoryForm();
        ...
        classForm.addEventListener('submit', async (e) => {
            ...
            const monthlyFee = parseFloat(document.getElementById('classMonthlyFee').value);
            const cls = {
                name: document.getElementById('className').value,
                teacherId: parseInt(document.getElementById('classTeacher').value),
                subjects: subjectsArray,
                monthlyFee: monthlyFee // Store monthly fee with the class
            };
            try {
                let savedClassId;
                if (classId) {
                    ...
                    request.onsuccess = async () => {
                        if (request.result) {
                            ...
                            savedClassId = await addData(STORE_CLASSES, cls); // Get the ID of the newly added class
                            alert('نئی کلاس شامل ہو گئی۔');
                        }
                        // If a monthly fee is specified, add/update it in fee structures
                        if (savedClassId && monthlyFee > 0) {
                            const feeStructure = {
                                classId: savedClassId,
                                feeType: 'Monthly Fee', // Fixed to 'Monthly Fee'
                                amount: monthlyFee
                            };
                            await upsertFeeStructure(feeStructure); // Custom function to handle add/update
                        }
                        resetClassForm();
                        loadClasses();
                        loadDashboardStats();
                    ...
                    return; // Exit here to prevent duplicate execution from the outer try-catch
                }
                // For update path, also update fee structure if monthly fee is specified
                if (savedClassId && monthlyFee > 0) {
                    const feeStructure = {
                        classId: savedClassId,
                        feeType: 'Monthly Fee',
                    ...
                    await upsertFeeStructure(feeStructure);
                } else if (savedClassId && monthlyFee === 0) {
                    // If monthly fee is set to 0, attempt to remove 'Monthly Fee' structure for this class
                    const existingFeeStructures = await getAll(STORE_FEE_STRUCTURES, 'classId', savedClassId);
                    const monthlyFeeStructure = existingFeeStructures.find(fs => fs.feeType === 'Monthly Fee');
                    if (monthlyFeeStructure) {
                        await deleteData(STORE_FEE_STRUCTURES, monthlyFeeStructure.id);
                    }
                }
                resetClassForm();
                loadClasses();
        ...
        // Add a helper function to upsert fee structures
        async function upsertFeeStructure(newFeeStructure) {
            try {
                const store = await getObjectStore(STORE_FEE_STRUCTURES, 'readwrite');
                const index = store.index('classId_feeType');
                const keyRange = IDBKeyRange.only([newFeeStructure.classId, newFeeStructure.feeType]);
                const request = index.get(keyRange);
                return new Promise((resolve, reject) => {
                    request.onsuccess = async () => {
                        const existing = request.result;
                        if (existing) {
                            existing.amount = newFeeStructure.amount;
                            const updateReq = store.put(existing);
                            updateReq.onsuccess = () => resolve(existing.id);
                            updateReq.onerror = reject;
                        } else {
                            const addReq = store.add(newFeeStructure);
        ...
        async function editClass(id) {
            try {
                ...
                if (cls) {
                    ...
                    // Load monthly fee
                    document.getElementById('classMonthlyFee').value = cls.monthlyFee || 0;
                    showModule('classes');
                    window.scrollTo(0, 0);
                }
            } catch (error) {
        ...
        feeStructureForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            let feeTypeValue = document.getElementById('feeType').value;
            if (feeTypeValue === 'custom') {
                feeTypeValue = document.getElementById('customFeeType').value.trim();
                if (!feeTypeValue) {
                    alert('براہ کرم کسٹم فیس کا نام درج کریں۔');
                    return;
                }
            }
            ...
            try {
                ...
                request.onsuccess = async () => {
                    ...
                    if (structureId) {
                        ...
                        alert('فیس اسٹرکچر اپ ڈیٹ ہو گئی۔');
                    } else {
                        const newId = await addData(STORE_FEE_STRUCTURES, structure);
                        savedStructure = { ...structure, id: newId };
                        alert('نیا فیس اسٹرکچر شامل ہو گیا۔');
                    }
                    await updateMonthlyDuesForFeeStructureChange(savedStructure);
        ...
        async function generateMonthlyDues() {
            console.log("Checking if recurring dues need to be generated...");
            const today = new Date();
            const currentMonth = today.getMonth() + 1;
            const currentMonthYear = `${today.getFullYear()}-${String(currentMonth).padStart(2, '0')}`;
            const lastCheck = localStorage.getItem('duesGeneratedForMonth');
            if (lastCheck === currentMonthYear) {
                console.log(`Dues for ${currentMonthYear} have already been checked/generated.`);
                return;
            }
            console.log(`Generating recurring dues for ${currentMonthYear}...`);
            try {
                const allFeeStructures = await getAll(STORE_FEE_STRUCTURES);
                const monthlyFees = allFeeStructures.filter(fs => fs.feeType === 'Monthly Fee');
                const yearlyFees = allFeeStructures.filter(fs => fs.feeType === 'Yearly Fee');
                if (monthlyFees.length === 0 && yearlyFees.length === 0) {
                    console.log("No recurring fee structures found.");
                    localStorage.setItem('duesGeneratedForMonth', currentMonthYear);
                    return;
                }
                const allStudents = await getAll(STORE_STUDENTS);
                const dueStore = await getObjectStore('monthlyDues', 'readwrite');
                let duesCreatedCount = 0;
                let feesToProcess = [...monthlyFees];
                if (currentMonth === 1) {
                    feesToProcess.push(...yearlyFees);
                }
                for (const fee of feesToProcess) {
                    const studentsInClass = allStudents.filter(s => s.classId === fee.classId);
                    for (const student of studentsInClass) {
                        ...
                        const index = dueStore.index('studentId_feeType_monthYear');
                        const checkReq = index.get([student.id, fee.feeType, currentMonthYear]);
                        const existingDue = await new Promise(resolve => { checkReq.onsuccess = () => resolve(checkReq.result); });
                        if (!existingDue) {
                            await new Promise(resolve => { dueStore.add(newDue).onsuccess = resolve; });
                            duesCreatedCount++;
                        }
                    }
                }
                if (duesCreatedCount > 0) {
                    console.log(`${duesCreatedCount} new recurring dues for ${currentMonthYear} have been generated.`);
                }
                localStorage.setItem('duesGeneratedForMonth', currentMonthYear);
                console.log("Finished generating recurring dues.");
            } catch (error) {
                console.error("Failed to generate recurring dues:", error);
            }
        }
        ...
        async function editFeeStructure(id) {
            try {
                ...
                if (structure) {
                    ...
                    const customFeeTypeInput = document.getElementById('customFeeType');
                    const standardOptions = Array.from(feeTypeSelect.options).map(opt => opt.value);
                    if (standardOptions.includes(structure.feeType)) {
                        feeTypeSelect.value = structure.feeType;
                        customFeeTypeInput.style.display = 'none';
                        customFeeTypeInput.value = '';
                    } else {
                        feeTypeSelect.value = 'custom';
                        customFeeTypeInput.value = structure.feeType;
                        customFeeTypeInput.style.display = 'block';
                    }
        ...
        feeCollectionForm.addEventListener('submit', async (e) => {
            ...
            const dueId = parseInt(document.getElementById('feeCollectionType').value);
            ...
            if (!studentId || !dueId || isNaN(paidAmount) || paidAmount < 0) {
                alert('براہ کرم طالب علم، واجب الادا فیس، اور ادا شدہ رقم درست درج کریں۔');
                return;
            }
            try {
                const dueRecord = await getById('monthlyDues', dueId);
                ...
                const feeRecord = {
                    ...
                    feeType: `${dueRecord.feeType} - ${dueRecord.monthYear}`,
                ...
                dueRecord.amountPaid = (dueRecord.amountPaid || 0) + paidAmount;
                dueRecord.discount = (dueRecord.discount || 0) + discount;
                if (dueRecord.amountPaid + dueRecord.discount >= (dueRecord.amountDue || dueRecord.feeAmount || 0)) {
                    dueRecord.status = 'paid';
                } else {
                    dueRecord.status = 'partial';
                }
                await updateData('monthlyDues', dueRecord);
                alert('فیس کامیابی سے وصول ہو گئی۔');
                if (typeof addFeeIncomeToTransactions === 'function') {
                    await addFeeIncomeToTransactions(feeRecord);
                }
        ...
        async function loadStudentFeeDetails(selectedStudentId = null) {
            ...
            feeCollectionTypeSelect.innerHTML = '<option value="">-- منتخب کریں --</option>';
            ...
            feeHistoryTableBody.innerHTML = '<tr><td colspan="5">لوڈ ہو رہا ہے...</td></tr>';
            dueFeesList.innerHTML = '<li>لوڈ ہو رہا ہے...</li>';
            try {
                const studentDues = await getAll('monthlyDues', 'studentId', studentId);
                const outstandingDues = studentDues.filter(d => d.status !== 'paid');
                dueFeesList.innerHTML = '';
                feeCollectionTypeSelect.innerHTML = '<option value="">-- فیس کی قسم منتخب کریں --</option>';
                if (outstandingDues.length > 0) {
                    outstandingDues.sort((a, b) => a.monthYear.localeCompare(b.monthYear)).forEach(due => {
                        const totalFee = due.amountDue || due.feeAmount || 0;
                        const dueAmount = totalFee - (due.amountPaid || 0) - (due.discount || 0);
                        const feeLabel = `${due.feeType} - ${due.monthYear}`;
                        const li = document.createElement('li');
                        li.textContent = `${feeLabel}: ${totalFee.toLocaleString()} (بقایا: ${dueAmount.toLocaleString()})`;
                        dueFeesList.appendChild(li);
                        const option = document.createElement('option');
                        option.value = due.id;
                        option.textContent = `${feeLabel} (${dueAmount.toLocaleString()} بقایا)`;
                        feeCollectionTypeSelect.appendChild(option);
                    });
                } else {
                    dueFeesList.innerHTML = '<li>کوئی فیس واجب الادا نہیں۔</li>';
                }
                feeCollectionTypeSelect.disabled = outstandingDues.length === 0;
                const feePayments = await getAll(STORE_FEES, 'studentId', studentId);
                feeHistoryTableBody.innerHTML = '';
                if (feePayments.length > 0) {
                    feePayments.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(payment => {
                        const row = feeHistoryTableBody.insertRow();
                        row.innerHTML = `
                    <td>${payment.date}</td>
                    <td>${payment.feeType}</td>
                    <td>${(payment.paidAmount || 0).toLocaleString()}</td>
                    <td>${(payment.discount || 0).toLocaleString()}</td>
                    <td><button class="delete" onclick="deleteFeeRecord(${payment.id})">حذف کریں</button></td>
                `;
                    });
                } else {
                    feeHistoryTableBody.innerHTML = '<tr><td colspan="5">کوئی فیس جمع نہیں کرائی گئی۔</td></tr>';
                }
            } catch (error) {
                console.error("Error loading student fee details:", error);
            }
        }
        async function deleteFeeRecord(feeId) {
            if (!confirm('کیا آپ واقعی اس فیس ریکارڈ کو حذف کرنا چاہتے ہیں؟ یہ واجب الادا رقم کو بھی اپ ڈیٹ کر دے گا۔')) {
                return;
            }
            try {
                const feeStore = await getObjectStore(STORE_FEES, 'readwrite');
                const feeRecord = await new Promise((res, rej) => {
                    const req = feeStore.get(feeId);
                    req.onsuccess = () => res(req.result);
                    req.onerror = (e) => rej(e.target.error);
                });
                if (!feeRecord || !feeRecord.dueId) {
                    await deleteData(STORE_FEES, feeId);
                    alert('فیس ریکارڈ (بغیر ڈیو لنک کے) حذف ہو گیا۔');
                    await loadStudentFeeDetails(feeStudentSelect.value);
                    return;
                }
                const dueStore = await getObjectStore(STORE_MONTHLY_DUES, 'readwrite');
                const dueRecord = await new Promise((res, rej) => {
                    const req = dueStore.get(feeRecord.dueId);
                    req.onsuccess = () => res(req.result);
                    req.onerror = (e) => rej(e.target.error);
                });
                if (dueRecord) {
                    dueRecord.amountPaid = (dueRecord.amountPaid || 0) - feeRecord.paidAmount;
                    dueRecord.discount = (dueRecord.discount || 0) - feeRecord.discount;
                    if (dueRecord.amountPaid <= 0 && dueRecord.discount <= 0) {
                        dueRecord.status = 'due';
                        dueRecord.amountPaid = 0;
                        dueRecord.discount = 0;
                    } else {
                        dueRecord.status = 'partial';
                    }
                    await new Promise((res, rej) => {
                        const req = dueStore.put(dueRecord);
                        req.onsuccess = res;
                        req.onerror = (e) => rej(e.target.error);
                    });
                }
                await new Promise((res, rej) => {
                    const req = feeStore.delete(feeId);
                    req.onsuccess = res;
                    req.onerror = (e) => rej(e.target.error);
                });
                alert('فیس ریکارڈ حذف ہو گیا اور واجب الادا رقم اپ ڈیٹ ہو گئی۔');
                await loadStudentFeeDetails(feeStudentSelect.value);
        ...
        async function loadStaffForAttendance() {
            ...
            const tableBody = document.getElementById('staffAttendanceTable').querySelector('tbody');
            tableBody.innerHTML = '<tr><td colspan="2">لوڈ ہو رہا ہے...</td></tr>';
            if (!date) {
                tableBody.innerHTML = '<tr><td colspan="2">براہ کرم تاریخ منتخب کریں.</td></tr>';
                return;
            }
            try {
                const allStaff = await getAll(STORE_TEACHERS);
                const attendanceRecords = await getAll('staffAttendance', 'date_staffId');
                const todaysRecords = attendanceRecords.filter(rec => rec.date === date);
                const attendanceMap = todaysRecords.reduce((map, record) => {
                    map[record.staffId] = record.status;
                    return map;
                }, {});
                tableBody.innerHTML = '';
                allStaff.sort((a, b) => a.name.localeCompare(b.name)).forEach(staff => {
                    const status = attendanceMap[staff.id] || 'Present';
                    const row = tableBody.insertRow();
                    row.innerHTML = `
                <td>${staff.name}</td>
                <td>
                    <label><input type="radio" name="status_${staff.id}" value="Present" ${status === 'Present' ? 'checked' : ''}> حاضر</label>
                    <label><input type="radio" name="status_${staff.id}" value="Absent" ${status === 'Absent' ? 'checked' : ''}> غیر حاضر</label>
                    <label><input type="radio" name="status_${staff.id}" value="Leave" ${status === 'Leave' ? 'checked' : ''}> رخصت</label>
                </td>
            `;
                    row.dataset.staffId = staff.id;
                });
            } catch (error) {
                console.error("Error loading staff for attendance:", error);
                tableBody.innerHTML = '<tr><td colspan="2">اسٹاف لوڈ کرنے میں خرابی۔</td></tr>';
            }
        }
        async function saveStaffAttendance() {
            const date = document.getElementById('staffAttendanceDate').value;
            const rows = document.getElementById('staffAttendanceTable').querySelectorAll('tbody tr');
            if (!date || rows.length === 0) {
                alert('براہ کرم تاریخ منتخب کریں اور اسٹاف موجود ہوں۔');
                return;
            }
            try {
                const store = await getObjectStore('staffAttendance', 'readwrite');
                for (const row of rows) {
                    const staffId = parseInt(row.dataset.staffId);
                    const status = row.querySelector('input[type="radio"]:checked').value;
                    const record = { date, staffId, status };
                    const index = store.index('date_staffId');
                    const request = index.get([date, staffId]);
                ...
                alert('اسٹاف کی حاضری محفوظ ہو گئی۔');
            } catch (error) {
        ...
        async function generateSalaryPaymentReport(startDate, endDate) {
            let expenses = await getAll(STORE_EXPENSES);
            let salaryPayments = expenses.filter(exp => {
                const isSalary = exp.recordType === 'expense' && exp.category === 'Salary';
                const dateMatch = (!startDate || exp.date >= startDate) && (!endDate || exp.date <= endDate);
                return isSalary && dateMatch;
            });
            if (salaryPayments.length === 0) return '<p>اس مدت میں کوئی تنخواہ کی ادائیگی نہیں ملی۔</p>';
            let tableHTML = `<table border="1" style="width:100%; border-collapse: collapse;">
                         <thead><tr><th>ادائیگی کی تاریخ</th><th>استاد کا نام</th><th>مہینہ</th><th>رقم</th><th>نوٹس</th></tr></thead>
                         <tbody>`;
            let totalPaid = 0;
            salaryPayments.sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(p => {
                const paymentMonth = p.description ? p.description.replace('Salary for ', '') : 'N/A';
                tableHTML += `<tr>
                         <td>${p.date}</td>
                         <td>${p.paidTo}</td>
                         <td>${paymentMonth}</td>
            ...
            tableHTML += `</tbody><tfoot>
                    <tr><td colspan="3"><b>کل ادا شدہ</b></td><td><b>${totalPaid.toLocaleString()}</b></td><td></td></tr>
                  </tfoot></table>`;
            return tableHTML;
        }
            ...
            try {
                ...
                switch (reportType) {
                    ...
                    case 'expense_report':
                        ...
                        break;
                    case 'salary_payment':
                        html += await generateSalaryPaymentReport(startDate, endDate);
                        break;
                    case 'student_list':
    ...
    `;
        ...
        let advancedReportRecipients = [];
        async function loadAdvancedFilterData() {
            const classSelect = document.getElementById('advClassFilter');
            if (classSelect.options.length <= 1) {
                const classes = await getAll(STORE_CLASSES);
                classes.sort((a, b) => a.name.localeCompare(b.name)).forEach(c => classSelect.add(new Option(c.name, c.id)));
            }
            updateStudentFilterByClass();
            const monthSelect = document.getElementById('advReportMonths');
            if (monthSelect.options.length === 0) {
                const urduMonths = ["جنوری", "فروری", "مارچ", "اپریل", "مئی", "جون", "جولائی", "اگست", "ستمبر", "اکتوبر", "نومبر", "دسمبر"];
                urduMonths.forEach((month, index) => monthSelect.add(new Option(month, index + 1)));
            }
            const yearSelect = document.getElementById('advReportYears');
        ...
        async function updateStudentFilterByClass() {
            const classId = document.getElementById('advClassFilter').value;
            const studentSelect = document.getElementById('advStudentFilter');
            studentSelect.innerHTML = '';
            let students = await getAll(STORE_STUDENTS);
            if (classId) {
                students = students.filter(s => s.classId === parseInt(classId));
            }
            students.sort((a, b) => a.name.localeCompare(b.name)).forEach(s => studentSelect.add(new Option(`${s.name} (${s.admissionNo})`, s.id)));
        }
        function updateAdvancedFilterVisibility() {
            const reportType = document.getElementById('advReportType').value;
            const show = (id, type = 'block') => document.getElementById(id).style.display = type;
            const hide = (id) => document.getElementById(id).style.display = 'none';
            if (!reportType) {
                hide('advDateControls'); hide('advFinancialTypeDiv'); hide('advClassFilterDiv'); hide('advStudentFilterDiv');
                return;
            }
            show('advDateControls', 'grid');
            show('advClassFilterDiv');
            show('advStudentFilterDiv');
            reportType === 'financial' ? show('advFinancialTypeDiv') : hide('advFinancialTypeDiv');
        }
        function getDateChecker() {
            const selectedMonths = Array.from(document.getElementById('advReportMonths').selectedOptions).map(opt => parseInt(opt.value));
            const selectedYears = Array.from(document.getElementById('advReportYears').selectedOptions).map(opt => parseInt(opt.value));
        ...
        function triggerSingleComm(id, type) {
            const recipient = [{ id: parseInt(id), type: type }];
            const reportType = document.getElementById('advReportType').value;
            const financialType = document.getElementById('advFinancialType').value;
            let messageType = 'custom';
            if (reportType === 'financial' && financialType === 'fee_due') messageType = 'fee_reminder';
            if (reportType === 'attendance') messageType = 'attendance_absent';
            if (reportType === 'marks') messageType = 'exam_result';
            launchAndPopulateCommModal(recipient, messageType);
        }
        async function generateAdvancedReport() {
            const reportType = document.getElementById('advReportType').value;
            const outputDiv = document.getElementById('advancedReportOutput');
            const commBtn = document.getElementById('commFromReportBtn');
            outputDiv.innerHTML = '<p>رپورٹ تیار کی جا رہی ہے...</p>';
            commBtn.style.display = 'none';
            advancedReportRecipients = [];
            if (!reportType) {
                outputDiv.innerHTML = '<p>براہ کرم رپورٹ کی قسم منتخب کریں۔</p>';
                return;
            }
            try {
                let reportData = { html: '', recipients: [] };
                switch (reportType) {
                    case 'financial':
                        reportData = await generateAdvancedFinancialReport();
                        break;
                    case 'attendance':
                        reportData = await generateAdvancedAttendanceReport();
                        break;
                    case 'marks':
                        reportData = await generateAdvancedMarksReport();
                        break;
                }
                outputDiv.innerHTML = reportData.html || '<p>اس معیار پر کوئی ڈیٹا نہیں ملا۔</p>';
                advancedReportRecipients = reportData.recipients || [];
                if (advancedReportRecipients.length > 0) {
                    commBtn.style.display = 'inline-block';
                }
            } catch (error) {
                console.error("Error generating advanced report:", error);
                outputDiv.innerHTML = `<p>رپورٹ بنانے میں خرابی: ${error.message}</p>`;
            }
        }
        async function generateAdvancedFinancialReport() {
            const financialType = document.getElementById('advFinancialType').value;
            const classId = document.getElementById('advClassFilter').value;
            const studentIds = Array.from(document.getElementById('advStudentFilter').selectedOptions).map(opt => parseInt(opt.value));
            const dateChecker = getDateChecker();
            let allStudents = await getAll(STORE_STUDENTS);
            if (classId) allStudents = allStudents.filter(s => s.classId === parseInt(classId));
            if (studentIds.length > 0) allStudents = allStudents.filter(s => studentIds.includes(s.id));
            let html = `<h4>${financialType === 'fee_due' ? 'واجب الادا فیس' : 'وصول شدہ فیس'} رپورٹ</h4><table width="100%" border="1" style="border-collapse: collapse;"><thead><tr><th>نام</th><th>کلاس</th><th>تفصیل</th><th>رقم</th><th>عمل</th></tr></thead><tbody>`;
            let recipients = [];
            let totalAmount = 0;
            if (financialType === 'fee_due') {
                const feeStructures = await getAll(STORE_FEE_STRUCTURES);
                const feePayments = await getAll(STORE_FEES);
                for (const student of allStudents) {
                    const studentFeeStructures = feeStructures.filter(fs => fs.classId === student.classId);
                    let totalDueForStudent = 0;
                    let dueDetails = [];
                    for (const structure of studentFeeStructures) {
                        const paidForType = feePayments.filter(p => p.studentId === student.id && p.feeType === structure.feeType).reduce((sum, p) => sum + p.paidAmount + p.discount, 0);
                        if (structure.amount - paidForType > 0) {
                            totalDueForStudent += structure.amount - paidForType;
                            dueDetails.push(`${structure.feeType}: ${(structure.amount - paidForType).toLocaleString()}`);
                        }
                    }
                    if (totalDueForStudent > 0) {
                        const actionBtn = student.contact ? `<button onclick="triggerSingleComm(${student.id}, 'students')">پیغام</button>` : 'N/A';
                        html += `<tr><td>${student.name}</td><td>${await getClassName(student.classId)}</td><td>${dueDetails.join(', ')}</td><td>${totalDueForStudent.toLocaleString()}</td><td>${actionBtn}</td></tr>`;
                        totalAmount += totalDueForStudent;
                        if (student.contact) recipients.push({ id: student.id, type: 'students' });
                    }
                }
            } else {
                const feePayments = await getAll(STORE_FEES);
                for (const student of allStudents) {
                    const paymentsInDate = feePayments.filter(p => p.studentId === student.id && dateChecker(p.date));
                    if (paymentsInDate.length > 0) {
                        let totalPaidByStudent = 0;
                        let paidDetails = paymentsInDate.map(p => { totalPaidByStudent += p.paidAmount; return `${p.date} (${p.feeType}): ${p.paidAmount.toLocaleString()}`; }).join('<br>');
                        const actionBtn = student.contact ? `<button onclick="triggerSingleComm(${student.id}, 'students')">پیغام</button>` : 'N/A';
                        html += `<tr><td>${student.name}</td><td>${await getClassName(student.classId)}</td><td>${paidDetails}</td><td>${totalPaidByStudent.toLocaleString()}</td><td>${actionBtn}</td></tr>`;
                        totalAmount += totalPaidByStudent;
                        if (student.contact) recipients.push({ id: student.id, type: 'students' });
                    }
                }
            }
            html += `</tbody><tfoot><tr><td colspan="4"><b>کل</b></td><td><b>${totalAmount.toLocaleString()}</b></td></tr></tfoot></table>`;
            return { html, recipients };
        }
        async function generateAdvancedAttendanceReport() {
            ...
            if (classId) allStudents = allStudents.filter(s => s.classId === parseInt(classId));
            if (studentIds.length > 0) allStudents = allStudents.filter(s => studentIds.includes(s.id));
            const attendanceRecords = await getAll(STORE_ATTENDANCE);
            let html = `<h4>حاضری کا خلاصہ</h4><table width="100%" border="1" style="border-collapse: collapse;"><thead><tr><th>نام</th><th>کلاس</th><th>کل دن</th><th>حاضر</th><th>غیر حاضر</th><th>فیصد</th><th>عمل</th></tr></thead><tbody>`;
            let recipients = [];
            for (const student of allStudents) {
                const studentRecords = attendanceRecords.filter(r => r.studentId === student.id && dateChecker(r.date));
                if (studentRecords.length === 0) continue;
                const totalDays = studentRecords.length;
                const presentDays = studentRecords.filter(r => r.status === 'Present').length;
                const actionBtn = student.contact ? `<button onclick="triggerSingleComm(${student.id}, 'students')">پیغام</button>` : 'N/A';
                html += `<tr><td>${student.name}</td><td>${await getClassName(student.classId)}</td><td>${totalDays}</td><td>${presentDays}</td><td>${totalDays - presentDays}</td><td>${((presentDays / totalDays) * 100).toFixed(1)}%</td><td>${actionBtn}</td></tr>`;
                if (student.contact) recipients.push({ id: student.id, type: 'students' });
            }
            html += '</tbody></table>';
            return { html, recipients };
        }
        async function generateAdvancedMarksReport() {
            const classId = document.getElementById('advClassFilter').value;
            const studentIds = Array.from(document.getElementById('advStudentFilter').selectedOptions).map(opt => parseInt(opt.value));
            let allStudents = await getAll(STORE_STUDENTS);
            if (classId) allStudents = allStudents.filter(s => s.classId === parseInt(classId));
            if (studentIds.length > 0) allStudents = allStudents.filter(s => studentIds.includes(s.id));
            const results = await getAll(STORE_RESULTS);
            let html = `<h4>نتائج کا خلاصہ</h4><table width="100%" border="1" style="border-collapse: collapse;"><thead><tr><th>نام</th><th>کلاس</th><th>امتحان</th><th>کل حاصل کردہ</th><th>کل نمبر</th><th>فیصد</th><th>عمل</th></tr></thead><tbody>`;
            let recipients = [];
            for (const student of allStudents) {
                const studentResults = results.filter(r => r.studentId === student.id);
                const resultsByExam = studentResults.reduce((acc, r) => {
                    acc[r.examName] = acc[r.examName] || { obtained: 0, total: 0 };
                    acc[r.examName].obtained += r.obtainedMarks;
                    acc[r.examName].total += r.totalMarks;
                    return acc;
                }, {});
                if (Object.keys(resultsByExam).length > 0) {
                    const actionBtn = student.contact ? `<button onclick="triggerSingleComm(${student.id}, 'students')">پیغام</button>` : 'N/A';
                    for (const examName in resultsByExam) {
                        const data = resultsByExam[examName];
                        const percentage = data.total > 0 ? ((data.obtained / data.total) * 100).toFixed(1) : 0;
                        html += `<tr><td>${student.name}</td><td>${await getClassName(student.classId)}</td><td>${examName}</td><td>${data.obtained}</td><td>${data.total}</td><td>${percentage}%</td><td>${actionBtn}</td></tr>`;
                    }
                    if (student.contact) recipients.push({ id: student.id, type: 'students' });
                }
            }
        ...
        function toggleCustomFeeType() {
            const feeTypeSelect = document.getElementById('feeType');
            const customFeeTypeInput = document.getElementById('customFeeType');
            if (feeTypeSelect.value === 'custom') {
                customFeeTypeInput.style.display = 'block';
                customFeeTypeInput.focus();
            } else {
                customFeeTypeInput.style.display = 'none';
                customFeeTypeInput.value = '';
            }
        }
        function launchAndPopulateCommModal(recipients, messageType) {
            const commModal = document.getElementById('communicationModal');
            if (!commModal) {
                alert('کمیونیکیشن ماڈیول نہیں ملا۔');
                return;
            }
            launchCommunicationModal();
            setTimeout(async () => {
                try {
                    const recipientType = recipients[0]?.type || 'students';
                    const commRecipientTypeSelect = document.getElementById('commRecipientType');
                    if (commRecipientTypeSelect) {
                        commRecipientTypeSelect.value = recipientType;
                        await handleRecipientTypeChange();
                    }
                    const recipientIds = recipients.map(r => r.id);
                    document.querySelectorAll('.comm-recipient-checkbox').forEach(cb => {
                        cb.checked = recipientIds.includes(parseInt(cb.value));
                    });
                    const commMessageTypeSelect = document.getElementById('commMessageType');
                    if (commMessageTypeSelect) {
                        commMessageTypeSelect.value = messageType;
                        handleMessageTypeChange();
                    }
                } catch (error) {
                    console.error("Error populating communication modal:", error);
                    alert("کمیونیکیشن ماڈیول کو آباد کرنے میں خرابی۔");
                }
            }, 300);
        }
        function triggerCommFromAdvancedReport() {
            if (!advancedReportRecipients || advancedReportRecipients.length === 0) {
                alert('پیغام بھیجنے کے لیے کوئی وصول کنندہ نہیں ہے۔');
                return;
            }
            const reportType = document.getElementById('advReportType').value;
            const financialType = document.getElementById('advFinancialType').value;
            ...
            if (reportType === 'attendance') messageType = 'attendance_absent';
            if (reportType === 'marks') messageType = 'exam_result';
            launchAndPopulateCommModal(advancedReportRecipients, messageType);
        }
        document.addEventListener('DOMContentLoaded', () => {
            const genReportBtn = document.getElementById('generateAdvancedReportBtn');
            if (genReportBtn) {
                genReportBtn.addEventListener('click', generateAdvancedReport);
            }
        });
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                await openDatabase();
                await generateRecurringDues();
    ...
    `;
                    ...
                    if (!table.dataset.pdfIconAdded) {
                        ...
                        icon.addEventListener('click', async () => {
                            try {
                                ...
                                const actionColumnIndex = Array.from(tableClone.querySelectorAll('thead th'))
                                    .findIndex(th => th.textContent.trim() === 'عمل');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const tableConfigs = [
                ...
                { id: 'transactionsTable', searchable: true, sortable: true, paginated: true }
            ];
        ...
        function createResultsModule() {
            ...
            if (resultsDiv.children.length > 0) return; resultsDiv.innerHTML = '';
    ...
    `;
        ...
        async function ensureQRCodeLibrary() {
            if (qrCodeLibraryLoaded && window.QRCode) return true;
            if (!window.QRCode) {
    ...
    `;
        ...
        <style>
            .id-card {
                width: 320px;
                height: 200px;
                background: #f0f0f0; border: 2px solid; border-color: #ffffff #808080 #808080 #ffffff;
                border-radius: 0;
        ...
        <div id="bulkIDCardResult" style="margin-top: 10px;">
            <table id="bulkIDCardTable" style="display: none;">
                ...
                <hr class="one" style="margin: 40px 0;">
        <h2>سرٹیفکیٹ جنریٹر</h2>
        <div class="form-grid">
            <div>
                <label for="certificateTypeSelect">سرٹیفکیٹ کی قسم</label>
                <select id="certificateTypeSelect" onchange="generateCertificate()">
                    <option value="hifz">تکمیلِ حفظ القرآن</option>
                    <option value="character">حسنِ اخلاق</option>
                    <option value="performance">حسنِ کارکردگی</option>
                    <option value="leaving">سکول چھوڑنے کا سرٹیفکیٹ</option>
                </select>
            </div>
            <div>
                <label for="certificateStudentSelect">طالب علم منتخب کریں</label>
                <select id="certificateStudentSelect" onchange="generateCertificate()" required>
                    <option value="">-- طالب علم منتخب کریں --</option>
                </select>
            </div>
            <div>
                <label for="certificateIssueDate">تاریخِ اجراء</label>
                <input type="date" id="certificateIssueDate" onchange="generateCertificate()">
            </div>
            <div>
                <label for="certificateFrameSelect">فریم منتخب کریں</label>
                <select id="certificateFrameSelect" onchange="generateCertificate()">
                    <option value="bgc.png">فریم 1</option>
                    <option value="bgc2.png">فریم 2</option>
                    <option value="bgc3.png">فریم 3</option>
                    <option value="bgc4.png">فریم 4</option>
                    <option value="bgc5.png">فریم 5</option>
                </select>
            </div>
        </div>
        <button class="print" onclick="printCertificate()" style="display: none;" id="printCertificateBtn">ایکسپورٹ کریں (PNG)</button>
        <div id="certificatePreviewContainer" class="printable" style="display: none; padding: 20px; background: #eee; margin-top: 20px;">
        </div>
       <hr class="one" style="margin: 40px 0;">
<h2>کسٹم سرٹیفکیٹ جنریٹر</h2>
<div class="form-grid">
    <div>
        <label for="customDataSource">ڈیٹا کا ماخذ (Data Source)</label>
        <select id="customDataSource" onchange="handleDataSourceChange()">
            <option value="students">موجودہ طلباء</option>
            <option value="csv">CSV / ٹیکسٹ ان پٹ</option>
        </select>
    </div>
</div>
<div id="internalDataSelector" class="form-grid full-width">
    <div>
        <label for="customBulkCertificateStudents">طلباء منتخب کریں (ایک یا زیادہ)</label>
        <select id="customBulkCertificateStudents" multiple size="5" style="height: 120px;"></select>
    </div>
</div>
<div id="csvDataSelector" class="form-grid full-width" style="display: none;">
    <div>
        <label for="csvDataInput">CSV ڈیٹا پیسٹ کریں (ہیڈر سمیت)</label>
        <textarea id="csvDataInput" rows="5" placeholder="FullName,Award,Date..."></textarea>
        <label for="csvFileInput" style="margin-top: 10px;">یا CSV فائل اپ لوڈ کریں</label>
        <input type="file" id="csvFileInput" accept=".csv" onchange="handleCSVFile(event)">
    </div>
</div>
<div class="form-grid"> <button onclick="generateCustomCertificates()">کسٹم سرٹیفکیٹس جنریٹ کریں</button>
    <button id="exportCustomCertificatesBtn" onclick="exportCustomCertificates()" style="display: none; background-color: #28a745;">ایکسپورٹ کریں (ZIP)</button>
</div>
<div id="customCertificatePreviewContainer" class="printable" style="display: none; flex-wrap: wrap; gap: 20px; justify-content: center; padding: 20px; background: #eee; margin-top: 20px;">
</div>
<div class="form-grid">
    <div class="full-width">
        <label for="customCertificateTitle">سرٹیفکیٹ کا عنوان</label>
        <input type="text" id="customCertificateTitle" value="سند برائے حسنِ کارکردگی">
    </div>
    <div class="full-width">
        <label for="customCertificateBody">سرٹیفکیٹ کا متن</label>
        <textarea id="customCertificateBody" rows="5">تصدیق کی جاتی ہے کہ [name] ولد [father_name]، جماعت [class] کے طالب علم، نے [Event Name] میں نمایاں کارکردگی کا مظاہرہ کیا ہے۔</textarea>
        <div class="placeholders" style="font-size: 0.9em; color: #666; margin-top: 5px; direction: ltr; text-align: right;">
            <b>:پلیس ہولڈرز</b><br>
            طلباء کے لیے: <code>[name]</code>, <code>[father_name]</code>, <code>[class]</code>, <code>[admission_no]</code>, <code>[date]</code>, <code>[year]</code><br>
            CSV کے لیے: ہیڈر کے نام استعمال کریں, مثلاً <code>[FullName]</code>, <code>[Award]</code><br>
            کوئی بھی کسٹم ٹیکسٹ: <code>[کوئی بھی متن یہاں لکھیں]</code>
        </div>
    </div>
    <div>
        <label for="customCertificateIssueDate">تاریخِ اجراء</label>
        <input type="date" id="customCertificateIssueDate">
    </div>
    <div>
        <label for="customCertificateFrameSelect">فریم منتخب کریں</label>
        <select id="customCertificateFrameSelect">
            <option value="bgc.png">فریم 1</option>
            <option value="bgc2.png">فریم 2</option>
            <option value="bgc3.png">فریم 3</option>
            <option value="bgc4.png">فریم 4</option>
    ...
    `;
            ...
            window.tryLoadImage = async function (id, name, type, admissionNo = null) {
                const numericId = parseInt(id.toString().replace(/\D/g, '')) || 0;
                const paddedId = numericId.toString().padStart(4, '0');
                const idVariants = [];
                if (admissionNo) {
                    idVariants.push(admissionNo);
                }
                idVariants.push(
                    id.toString(),
                    `TCH${paddedId}`,
            ...
            window.generateSingleIDCard = async function () {
                ...
                try {
                    ...
                    const autoImage = await window.tryLoadImage(person.id, person.name, type, person.admissionNo);
                    const photoSrc = currentPhoto || autoImage || null;
                    let imageElementHTML;
                    if (photoSrc) {
                        imageElementHTML = `<img src="${photoSrc}" class="photo" alt="تصویر">`;
                    } else {
                        let qrData = `${idNumber}|${person.name}`;
                        if (type === 'student') {
                            qrData += `|C:${className.substring(0, 10)}`;
                        } else {
                            qrData += `|T`;
                        }
            ...
            window.generateBulkIDCards = async function () {
                ...
                try {
                    const storeName = type === 'student' ? window.STORE_STUDENTS : window.STORE_TEACHERS;
                    let allPersonsInStore = await window.getAll(storeName);
                    const selectedIds = Array.from(tbody.querySelectorAll('tr .bulk-select:checked'))
                    ...
                    const selectedPersons = allPersonsInStore.filter(p => selectedIds.includes(p.id));
                    const schoolSetting = await window.getById(window.STORE_SETTINGS, 'schoolName');
                    const schoolName = schoolSetting ? schoolSetting.value : 'مدرسہ کا نام';
                    tbody.innerHTML = '';
                    for (const person of selectedPersons.sort((a, b) => a.name.localeCompare(b.name))) {
                        const idNumber = type === 'student' ? person.admissionNo : `TCH${person.id.toString().padStart(4, '0')}`;
                        const currentClassName = type === 'student' ? (await window.getClassName(person.classId) || 'N/A') : 'N/A';
                        const autoImage = await window.tryLoadImage(person.id, person.name, type, person.admissionNo);
                        let imageElementHTMLBulk;
                        if (autoImage) {
                            if (document.querySelector("#bulkIdCardTemplate").value == "front") {
                                imageElementHTMLBulk = `<img src="${autoImage}" class="photo" alt="تصویر">`;
                            } else {
                                imageElementHTMLBulk = `<img src="logo.png" class="photo" alt="تصویر">`;
                            }
                        } else {
                            let qrData = `${idNumber}|${person.name}`;
                            if (type === 'student') {
                                qrData += `|C:${currentClassName.substring(0, 10)}`;
                            } else {
                                qrData += `|T`;
                            }
                            ...
                            if (qrCodeDataURL) {
                                if (document.querySelector("#bulkIdCardTemplate").value == "front") {
                                    ...
                                    imageElementHTMLBulk = `<img src="logo.png" class="photo qr-code-image" alt="QR Code">`;
                                }
                            } else {
                ...
                <td>${person.name}</td>
                <td>
                    <div class="preview" style="overflow:hidden; border: 1px solid #ccc;">
        ...
        function initializeNavigation() {
            ...
            NAV_CONFIG.forEach(item => {
                ...
                button.onclick = () => showModule(item.moduleId);
                navDesktop.appendChild(button);
            });
            mainNavContainer.appendChild(navDesktop);
            ...
            mainNavContainer.appendChild(navMobile);
            const oldStyle = document.head.querySelector('style[data-nav-mobile]');
        ...
        .hamburger { background-color: #4a7bdc; color: white; border: none; padding: 10px 15px; font-size: 1.2em; cursor: pointer; border-radius: 4px; float: right; }
        .mobile-menu { display: none; flex-direction: column; background-color: #5a8dee; padding: 10px; position: absolute; top: 100%; right: 0; width: 220px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2); z-index: 1001; clear: both; }
        .mobile-menu button { background-color: #4a7bdc; color: white; border: none; padding: 12px; margin: 5px 0; cursor: pointer; font-size: 1.1em; border-radius: 4px; text-align: right; }
        @media screen and (max-width: 768px) {
            #main-nav-container { position: relative; z-index: 999; text-align: right; padding: 10px; min-height: 55px; }
...
/* Script Block 4 */
        ...
        `;
            ...
            function dm_openModal() {
                localStorage.setItem('donationModalLastSeen', Date.now().toString());
                if (dm_triggerButton) dm_triggerButton.style.display = 'none';
            ...
            function dm_init() {
                const lastSeen = localStorage.getItem('donationModalLastSeen');
                if (lastSeen) {
                    const thirtyDaysInMillis = 30 * 24 * 60 * 60 * 1000;
                    if (Date.now() - parseInt(lastSeen) < thirtyDaysInMillis) {
                        return;
                    }
                }
                ...
                try {
                    ...
                    setTimeout(() => {
                        document.querySelectorAll(".comm-recipient-item label").forEach(label => {
                            ...
                            const match = text.match(/(?:\+92|0092|92|0)?\d{3}[-\s.]?\d{7}/);
                            if (match) {
                                const cleaned = match[0].replace(/[^+\d]/g, '');
                                const nameClass = text.replace(match[0], '').trim().replace(/[-\s]+$/, '');
                                label.innerHTML = `${nameClass}<br><small style="color:#555">${cleaned}</small>`;
                            }
                        });
                    }, 500);
        ...
        async function generateCertificate() {
            const studentId = parseInt(document.getElementById('certificateStudentSelect').value);
            const certType = document.getElementById('certificateTypeSelect').value;
            const issueDateEl = document.getElementById('certificateIssueDate');
            const selectedFrame = document.getElementById('certificateFrameSelect').value;
            ...
            try {
                ...
                switch (certType) {
                    case 'hifz':
                        title = "سند تکمیلِ حفظ القرآن الكريم";
                        bodyText = `<p>تصدیق کی جاتی ہے کہ <span class="field">${student.name}</span> ولد <span class="field">${student.fatherName}</span>، جو ہمارے مدرسہ کے طالب علم ہیں، نے کامیابی کے ساتھ قرآنِ کریم کا حفظ مکمل کر لیا ہے۔ ہم ان کے لئے دنیا و آخرت میں کامیابی کی دعا کرتے ہیں۔</p>`;
                        break;
                    case 'character':
                        title = "سرٹیفکیٹ برائے حسنِ اخلاق";
                        bodyText = `<p>تصدیق کی جاتی ہے کہ <span class="field">${student.name}</span> ولد <span class="field">${student.fatherName}</span>، جماعت <span class="field">${className}</span> کے طالب علم، اپنے تعلیمی دور میں بہترین اخلاق اور عمدہ کردار کے حامل رہے۔</p>`;
                        break;
                    case 'performance':
                        title = "سرٹیفکیٹ برائے حسنِ کارکردگی";
                        bodyText = `<p>یہ سرٹیفکیٹ <span class="field">${student.name}</span> ولد <span class="field">${student.fatherName}</span>، جماعت <span class="field">${className}</span>، کو ان کی نمایاں تعلیمی کارکردگی اور مدرسہ کی سرگرمیوں میں بہترین حصہ لینے پر دیا جا رہا ہے۔</p>`;
                        break;
                    case 'leaving':
                        title = "سکول چھوڑنے کا سرٹیفکیٹ";
                        bodyText = `<p>تصدیق کی جاتی ہے کہ <span class="field">${student.name}</span> ولد <span class="field">${student.fatherName}</span>، جن کا داخلہ نمبر <span class="field">${student.admissionNo}</span> ہے، ہمارے مدرسہ میں زیرِ تعلیم رہے۔ ان کی تاریخِ داخلہ <span class="field">${student.admissionDate}</span> ہے۔ ہم ان کے اچھے چال چلن کی تصدیق کرتے ہیں اور ان کے مستقبل کے لیے نیک خواہشات کا اظہار کرتے ہیں۔</p>`;
                        break;
                }
                const certificateHTML = `
                    <div class="certificate-paper">
                        <div class="certificate-border" style="background-image: url('${selectedFrame}');">
                            <div class="certificate-inner-border">
                                <div class="cert-header">
                                    <p class="bismillah">بِسْمِ اللَّـهِ الرَّحْمَـٰنِ الرَّحِيمِ</p>
                                ...
                                <div class="cert-footer">
                                    <div class="date-section">
                                        <p>تاریخِ اجراء: ${new Date(issueDate).toLocaleDateString('ur-PK')}</p>
                                    </div>
                                    <div class="signature-section">
                                        <p class="signature-line">دستخط مدیر</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                alert('سرٹیفکیٹ بنانے میں خرابی: ' + error.message);
                previewContainer.style.display = 'none';
                printBtn.style.display = 'none';
            }
        }
        async function printCertificate() {
            const previewContainer = document.getElementById('certificatePreviewContainer');
            const certificateElement = previewContainer.querySelector('.certificate-paper');
            const printBtn = document.getElementById('printCertificateBtn');
            if (!certificateElement || previewContainer.style.display === 'none') {
                alert('پہلے ایکسپورٹ کرنے کے لیے ایک سرٹیفکیٹ بنائیں۔');
                return;
            }
            const originalButtonText = printBtn.textContent;
            printBtn.textContent = 'ایکسپورٹ ہو رہا ہے...';
            ...
            try {
                ...
                const link = document.createElement('a');
                const studentId = parseInt(document.getElementById('certificateStudentSelect').value);
                let filename = `Certificate_${Date.now()}.png`;
                if (studentId) {
                    const student = await getById(STORE_STUDENTS, studentId);
                    if (student) {
                        filename = `Certificate_${student.name.replace(/\s/g, '_')}.png`;
                    }
                }
                ...
                alert('سرٹیفکیٹ ایکسپورٹ کرنے میں خرابی۔');
            } finally {
                printBtn.textContent = originalButtonText;
                printBtn.disabled = false;
            }
        }
        const existingShowModule = window.showModule;
        window.showModule = function (moduleId) {
            existingShowModule(moduleId);
            if (moduleId === 'idCardGenerator') {
                loadStudentsForCertificate();
                loadStudentsForBulkCertificate();
                handleDataSourceChange();
            }
        };
        async function loadStudentsForBulkCertificate() {
            const select = document.getElementById('customBulkCertificateStudents');
            if (!select || select.options.length > 0) return;
            select.innerHTML = '';
            try {
                const students = await getAll(STORE_STUDENTS);
                students.sort((a, b) => a.name.localeCompare(b.name, 'ur')).forEach(student => {
                    const option = document.createElement('option');
                    option.value = student.id;
                    option.textContent = `${student.name} (${student.admissionNo})`;
                    select.appendChild(option);
                ...
                console.error("Error loading students for bulk certificate:", error);
            }
        }
        async function exportCustomCertificates() {
            const certificateElements = document.querySelectorAll('#customCertificatePreviewContainer .certificate-paper');
            const exportBtn = document.getElementById('exportCustomCertificatesBtn');
            if (certificateElements.length === 0) {
                alert('ایکسپورٹ کرنے کے لیے کوئی سرٹیفکیٹ نہیں ہے۔');
                return;
            }
            if (!window.JSZip || !window.html2canvas) {
                alert("ضروری لائبریریاں (JSZip, html2canvas) لوڈ نہیں ہیں۔ براہ کرم صفحہ کو ریفریش کریں۔");
                return;
            }
            exportBtn.textContent = 'ایکسپورٹ ہو رہا ہے...';
            exportBtn.disabled = true;
            try {
                const zip = new JSZip();
                for (const certElement of certificateElements) {
                    const studentName = certElement.dataset.studentName || `Certificate_${Date.now()}`;
                    const canvas = await html2canvas(certElement, { scale: 3, useCORS: true, backgroundColor: '#ffffff' });
                    const imgData = canvas.toDataURL('image/png').split(',')[1];
                    zip.file(`${studentName}.png`, imgData, { base64: true });
                }
                const content = await zip.generateAsync({ type: 'blob' });
                const link = document.createElement('a');
                link.href = URL.createObjectURL(content);
                link.download = `Custom_Certificates_${Date.now()}.zip`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(link.href);
            } catch (error) {
                console.error('Error exporting bulk custom certificates:', error);
                alert('سرٹیفکیٹس ایکسپورٹ کرنے میں خرابی۔');
            } finally {
                exportBtn.textContent = 'ایکسپورٹ کریں (ZIP)';
                exportBtn.disabled = false;
            }
        }
        function handleDataSourceChange() {
            const source = document.getElementById('customDataSource').value;
            document.getElementById('internalDataSelector').style.display = (source === 'students') ? 'block' : 'none';
            document.getElementById('csvDataSelector').style.display = (source === 'csv') ? 'block' : 'none';
        }
        ...
        async function generateCustomCertificates() {
            const dataSource = document.getElementById('customDataSource').value;
            const certTitle = document.getElementById('customCertificateTitle').value;
            const certBodyTemplate = document.getElementById('customCertificateBody').value;
            const issueDateEl = document.getElementById('customCertificateIssueDate');
            const selectedFrame = document.getElementById('customCertificateFrameSelect').value;
            const previewContainer = document.getElementById('customCertificatePreviewContainer');
            const exportBtn = document.getElementById('exportCustomCertificatesBtn');
            if (!issueDateEl.value) {
                issueDateEl.value = new Date().toISOString().split('T')[0];
            }
            ...
            if (dataSource === 'students') {
                const selectedOptions = Array.from(document.getElementById('customBulkCertificateStudents').selectedOptions);
                const studentIds = selectedOptions.map(opt => parseInt(opt.value));
                if (studentIds.length === 0) {
                    alert('براہ کرم کم از کم ایک طالب علم منتخب کریں۔');
                    return;
                }
                for (const id of studentIds) {
                    ...
                    if (student) {
                        records.push({
                            ...
                            class: await getClassName(student.classId),
                            admission_no: student.admissionNo,
                            filename: student.name.replace(/\s/g, '_')
                        });
                    }
                }
            } else if (dataSource === 'csv') {
                const csvText = document.getElementById('csvDataInput').value;
                if (!csvText.trim()) {
                    alert('CSV ڈیٹا ان پٹ خالی ہے۔');
                    return;
                }
                ...
                if (records.length > 0) {
                    ...
                    records.forEach(r => {
                        r.filename = r[firstHeader] ? r[firstHeader].replace(/\s/g, '_') : `record_${Date.now()}`;
                    });
                }
            }
            if (records.length === 0) {
                alert('سرٹیفکیٹ بنانے کے لیے کوئی ڈیٹا نہیں ملا۔');
                return;
            }
            previewContainer.innerHTML = '<p>سرٹیفکیٹس تیار کیے جا رہے ہیں...</p>';
            ...
            try {
                ...
                for (const record of records) {
                    const replacements = {
                        ...record,
                        date: new Date(issueDate).toLocaleDateString('ur-PK'),
                        year: issueYear
                    };
                    const finalBody = certBodyTemplate.replace(/\[([^\]]+)\]/g, (fullMatch, key) => {
                        const value = replacements[key] || key;
                        return `<span class="field">${value}</span>`;
                    });
                    allCertificatesHTML += `
                <div class="certificate-paper" data-student-name="${record.filename}">
                    <div class="certificate-border" style="background-image: url('${selectedFrame}');">
                        <div class="certificate-inner-border">
                            <div class="cert-header">
                                <p class="bismillah">بِسْمِ اللَّـهِ الرَّحْمَـٰنِ الرَّحِيمِ</p>
                                <h2 class="school-name">${schoolName}</h2>
                                <h3 class="cert-title">${certTitle}</h3>
                            </div>
                            <div class="cert-body"><p>${finalBody}</p></div>
                            <div class="cert-footer">
                                <div class="signature-section"><p class="signature-line">دستخط مدیر</p></div>
                                <div class="date-section"><p>تاریخِ اجراء: ${new Date(issueDate).toLocaleDateString('ur-PK')}</p></div>
                            </div>
                        </div>
                    </div>
                </div>`;
                }
                previewContainer.innerHTML = allCertificatesHTML;
                if (allCertificatesHTML) {
                    exportBtn.style.display = 'inline-block';
                }
            } catch (error) {
                console.error("Error generating custom certificates:", error);
                previewContainer.innerHTML = '<p>سرٹیفکیٹس بنانے میں خرابی: ' + error.message + '</p>';
            }
        }
        async function exportCustomCertificates() {
            const certificateElements = document.querySelectorAll('#customCertificatePreviewContainer .certificate-paper');
        ...
        const markingTypes = {
            letters: { Present: 'ح', Absent: 'غ', Leave: 'ر' },
            symbols: { Present: '✓', Absent: '✗', Leave: '—' }
        };
        function toggleAttendanceView() {
            const viewType = document.getElementById('attendanceViewType').value;
            const dailyView = document.getElementById('attendanceDailyView');
            const registerView = document.getElementById('attendanceRegisterView');
            const dailyControls = document.getElementById('attendanceDailyControls');
            const monthlyControls = document.getElementById('attendanceMonthlyControls');
            const markingTypeContainer = document.getElementById('attendanceMarkingTypeContainer');
            if (viewType === 'daily') {
                dailyView.style.display = 'block';
                dailyControls.style.display = 'block';
                registerView.style.display = 'none';
                monthlyControls.style.display = 'none';
                markingTypeContainer.style.display = 'none';
                ...
                markingTypeContainer.style.display = 'block';
                if (!document.getElementById('attendanceMonth').value) {
        ...
        async function loadStudentsForAttendance() {
            ...
            try {
                ...
                const keyRange = IDBKeyRange.only([date, classId]);
                const existingAttendance = await getAll(STORE_ATTENDANCE, 'date_classId', keyRange);
                const attendanceMap = existingAttendance.reduce((map, record) => {
                    map[record.studentId] = record.status;
                    return map;
                }, {});
                ...
                <td>${student.name}</td>
                <td>
                    <label><input type="radio" name="attendance_${student.id}" value="Present" data-student-id="${student.id}" ${existingStatus === 'Present' ? 'checked' : ''}> حاضر</label>
                    <label><input type="radio" name="attendance_${student.id}" value="Absent" data-student-id="${student.id}" ${existingStatus === 'Absent' ? 'checked' : ''}> غیر حاضر</label>
                    <label><input type="radio" name="attendance_${student.id}" value="Leave" data-student-id="${student.id}" ${existingStatus === 'Leave' ? 'checked' : ''}> رخصت</label>
                </td>
            `;
                });
            } catch (error) {
                console.error("Error loading students for daily attendance:", error);
                attendanceTableBody.innerHTML = '<tr><td colspan="3">حاضری کے لیے طلباء کو لوڈ کرنے میں خرابی۔</td></tr>';
            }
        }
        async function saveAttendance() {
            ...
            const recordsToSave = Array.from(attendanceRows).map(input => ({
                date: date,
                classId: classId,
                studentId: parseInt(input.dataset.studentId),
                status: input.value
        ...
        async function loadMonthlyRegister() {
            const classId = parseInt(attendanceClassSelect.value);
            const monthYear = document.getElementById('attendanceMonth').value;
            const dateRow = document.getElementById('dateRow');
            const tableBody = document.getElementById('attendanceRegisterBody');
            dateRow.innerHTML = '<th class="student-name">طالب علم کا نام</th>';
            tableBody.innerHTML = '';
            if (!classId || !monthYear) {
                tableBody.innerHTML = '<tr><td>براہ کرم مہینہ اور کلاس منتخب کریں۔</td></tr>';
                return;
            }
            tableBody.innerHTML = '<tr><td>لوڈ ہو رہا ہے...</td></tr>';
            ...
            dateRow.innerHTML += '<th class="total-cell">حاضر</th><th class="total-cell">فیصد</th>';
            try {
                const students = await getAll(STORE_STUDENTS, 'classId', classId);
                if (students.length === 0) {
                    tableBody.innerHTML = '<tr><td>اس کلاس میں کوئی طالب علم نہیں۔</td></tr>';
                    return;
                }
                const startDate = `${monthYear}-01`;
                const endDate = `${monthYear}-${daysInMonth}`;
                const range = IDBKeyRange.bound([startDate, classId], [endDate, classId]);
                const monthlyRecords = await getAll(STORE_ATTENDANCE, 'date_classId', range);
                ...
                students.sort((a, b) => a.name.localeCompare(b.name)).forEach(student => {
                    const row = tableBody.insertRow();
                    row.innerHTML = `<td class="student-name">${student.name}</td>`;
                    let presentCount = 0;
                    for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        cell.textContent = markingTypes[markingType][status] || markingTypes[markingType]['Absent'];
                        cell.onclick = () => updateCellAttendance(cell, student.id, classId, dateString, status);
                    }
                    row.insertCell().textContent = presentCount;
                    row.insertCell().textContent = `${Math.round((presentCount / daysInMonth) * 100)}%`;
                });
                calculateAndDisplayFooterTotals();
            } catch (error) {
                console.error("Error loading monthly register:", error);
                tableBody.innerHTML = '<tr><td>رجسٹر لوڈ کرنے میں خرابی۔</td></tr>';
            }
        }
        async function updateCellAttendance(cell, studentId, classId, date, currentStatus) {
            const statuses = ['Absent', 'Present', 'Leave'];
            ...
            cell.textContent = markingTypes[markingType][newStatus];
            cell.onclick = () => updateCellAttendance(cell, studentId, classId, date, newStatus);
            const row = cell.parentElement;
            const dayCount = row.cells.length - 3;
            let presentCount = 0;
            for (let i = 1; i <= dayCount; i++) {
                if (row.cells[i] && row.cells[i].classList.contains('Present')) {
                    presentCount++;
                }
            }
            row.cells[row.cells.length - 2].textContent = presentCount;
            row.cells[row.cells.length - 1].textContent = `${Math.round((presentCount / dayCount) * 100)}%`;
            calculateAndDisplayFooterTotals();
        }
        function calculateAndDisplayFooterTotals() {
            const tableBody = document.getElementById('attendanceRegisterBody');
            const tfoot = document.getElementById('attendanceRegisterFoot');
            const dateRow = document.getElementById('dateRow');
            if (!tableBody || !tfoot || !dateRow || tableBody.rows.length === 0 || tableBody.rows[0].cells.length <= 1) {
                return;
            }
            const summaryRow = tfoot.querySelector('.summary-row');
            summaryRow.innerHTML = '<td class="student-name">کل حاضری</td>';
            const dayColumnsCount = dateRow.cells.length - 3;
            for (let i = 1; i <= dayColumnsCount; i++) {
                let dailyPresentCount = 0;
                for (const row of tableBody.rows) {
                    const cell = row.cells[i];
                    if (cell && cell.classList.contains('Present')) {
                        dailyPresentCount++;
                    }
                }
                const totalCell = document.createElement('td');
                totalCell.className = 'total-cell';
                totalCell.textContent = dailyPresentCount;
            ...
            const promises = records.map(record => {
                return new Promise((resolve, reject) => {
                    ...
                    request.onsuccess = () => {
                        ...
                        if (existingRecord) {
                            finalRecord.id = existingRecord.id;
                        }
        ...
        async function printAttendanceRegister() {
            ...
            const className = attendanceClassSelect.options[attendanceClassSelect.selectedIndex]?.text || 'N/A';
            const printContainer = document.getElementById('attendancePrintContainer');
            const printTableThead = document.getElementById('attendancePrintTable').querySelector('thead');
            const printTableTbody = document.getElementById('attendancePrintTable').querySelector('tbody');
            printTableThead.innerHTML = '';
            printTableTbody.innerHTML = '';
            if (!classId) {
                alert('پرنٹ کرنے کے لیے براہ کرم کلاس منتخب کریں۔');
                return;
            }
            if (viewType === 'daily') {
                const date = attendanceDateInput.value;
                if (!date) { alert('پرنٹ کرنے کے لئے تاریخ منتخب کریں'); return; }
                document.getElementById('printAttendanceDate').textContent = date;
                document.getElementById('printAttendanceClass').textContent = className;
                printTableThead.innerHTML = `<tr><th>داخلہ نمبر</th><th>نام</th><th>حالت</th></tr>`;
                const dailyTbody = document.getElementById('attendanceTable').querySelector('tbody');
                printTableTbody.innerHTML = dailyTbody ? dailyTbody.innerHTML : '';
            } else {
                const monthYear = document.getElementById('attendanceMonth').value;
                if (!monthYear) { alert('پرنٹ کرنے کے لئے مہینہ منتخب کریں'); return; }
                document.getElementById('printAttendanceDate').textContent = monthYear;
                document.getElementById('printAttendanceClass').textContent = className;
                const registerThead = document.getElementById('dateRow');
                const registerTbody = document.getElementById('attendanceRegisterBody');
                printTableThead.innerHTML = registerThead ? registerThead.innerHTML : '';
                printTableTbody.innerHTML = registerTbody ? registerTbody.innerHTML : '';
            }
            const schoolNameSetting = await getById(STORE_SETTINGS, 'schoolName');
            const schoolName = schoolNameSetting ? schoolNameSetting.value : 'مدرسہ کا نام';
            const printHeader = document.createElement('h3');
        ...
        function showFeeSubModule(subModuleId) {
            const tabIdMap = {
                fees: 'feeCollectionTab',
                salaries: 'salaryPaymentTab'
            };
            const targetTabId = tabIdMap[subModuleId];
            if (!targetTabId) {
                console.error(`No tab ID found for submodule: ${subModuleId}`);
                return;
            }
            document.querySelectorAll('.sub-module').forEach(sub => {
            ...
            if (targetTab) {
                ...
                console.error(`Target tab element with ID ${targetTabId} not found.`);
                return;
            }
            document.querySelectorAll('.sub-nav button').forEach(btn => btn.classList.remove('active'));
            const activeButton = Array.from(document.querySelectorAll('.sub-nav button')).find(btn => btn.getAttribute('onclick').includes(`'${subModuleId}'`));
            if (activeButton) activeButton.classList.add('active');
            if (subModuleId === 'salaries') {
                loadTeachersForSalary();
                const monthInput = document.getElementById('salaryForMonth');
                if (!monthInput.value) {
                    monthInput.value = new Date().toISOString().slice(0, 7);
                }
                document.getElementById('salaryPaymentDate').value = new Date().toISOString().split('T')[0];
                loadTeacherSalaryDetails();
            }
        }
        async function loadTeachersForSalary() {
            await loadTeachersIntoSelect('salaryTeacherSelect', true);
        }
        async function loadTeacherSalaryDetails() {
            const teacherId = parseInt(document.getElementById('salaryTeacherSelect').value);
            const monthYear = document.getElementById('salaryForMonth').value;
            const detailsContainer = document.getElementById('teacherSalaryDetailsContainer');
            const historyTableBody = document.getElementById('salaryHistoryTable').querySelector('tbody');
            detailsContainer.style.display = 'none';
            historyTableBody.innerHTML = '<tr><td colspan="5">براہ کرم استاد اور مہینہ منتخب کریں۔</td></tr>';
            if (!teacherId || !monthYear) return;
            try {
                const teacher = await getById(STORE_TEACHERS, teacherId);
                if (!teacher) throw new Error('Teacher not found');
                document.getElementById('salaryDetailsHeader').textContent = `${teacher.name} - ${monthYear}`;
                const designatedSalary = teacher.salary || 0;
                document.getElementById('teacherDesignatedSalary').textContent = designatedSalary.toLocaleString();
                const allExpenses = await getAll(STORE_EXPENSES);
                const teacherSalaryPayments = allExpenses.filter(exp =>
                    exp.recordType === 'expense' && exp.category === 'Salary' && exp.paidTo === teacher.name
                );
                const paymentsForMonth = teacherSalaryPayments.filter(p => p.description && p.description.includes(monthYear));
                const totalPaidForMonth = paymentsForMonth.reduce((sum, p) => sum + p.amount, 0);
                document.getElementById('paidForMonth').textContent = totalPaidForMonth.toLocaleString();
                const balance = designatedSalary - totalPaidForMonth;
                document.getElementById('balanceForMonth').textContent = balance.toLocaleString();
                document.getElementById('balanceForMonth').style.color = balance > 0 ? 'red' : 'green';
                detailsContainer.style.display = 'block';
                historyTableBody.innerHTML = '';
                if (teacherSalaryPayments.length > 0) {
                    teacherSalaryPayments.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(p => {
                    ...
                    <td>${p.notes || '-'}</td>
                    <td>
                        <button class="print" onclick="printSalarySlip(${p.id})">پرنٹ</button>
                        <button class="delete" onclick="deleteSalaryPayment(${p.id})">حذف</button>
                    </td>
                `;
                    });
                } else {
                    historyTableBody.innerHTML = '<tr><td colspan="5">کوئی ادائیگی ریکارڈ نہیں ملی۔</td></tr>';
                }
            } catch (error) {
                console.error("Error loading teacher salary details:", error);
            }
        }
        document.getElementById('salaryPaymentForm').addEventListener('submit', async (e) => {
            ...
            if (!teacherId || !monthYear || !amount || amount <= 0) {
                alert('براہ کرم استاد، مہینہ اور درست رقم منتخب کریں۔');
                return;
            }
            try {
                const teacher = await getById(STORE_TEACHERS, teacherId);
                ...
                const expenseRecord = {
                    ...
                    category: 'Salary',
                    amount: amount,
                    date: document.getElementById('salaryPaymentDate').value,
                    paidTo: teacher.name,
                    description: `Salary for ${monthYear}`,
                    notes: document.getElementById('salaryNotes').value.trim()
                };
                const newExpenseId = await addData(STORE_EXPENSES, expenseRecord);
                alert(`${teacher.name} کو ${amount.toLocaleString()} کی تنخواہ کامیابی سے ادا کر دی گئی۔`);
                document.getElementById('salaryPaymentForm').reset();
                document.getElementById('salaryPaymentDate').value = new Date().toISOString().split('T')[0];
                await loadTeacherSalaryDetails();
                await loadDashboardStats();
                if (confirm('کیا آپ ادائیگی کی سلپ پرنٹ کرنا چاہتے ہیں؟')) {
                    printSalarySlip(newExpenseId);
                }
            } catch (error) {
                console.error("Error paying salary:", error);
            }
        });
        async function deleteSalaryPayment(expenseId) {
            if (confirm('کیا آپ واقعی اس تنخواہ کی ادائیگی کو حذف کرنا چاہتے ہیں؟')) {
                try {
                    await deleteData(STORE_EXPENSES, expenseId);
                    alert('ادائیگی کامیابی سے حذف کر دی گئی۔');
                    await loadTeacherSalaryDetails();
                    await loadDashboardStats();
                } catch (error) {
        ...
        async function updateMonthlyDuesForFeeStructureChange(feeStructure) {
            if (feeStructure.feeType !== 'Monthly Fee' && feeStructure.feeType !== 'Yearly Fee') {
                return;
            }
            const today = new Date();
            const currentYear = today.getFullYear();
            const currentMonth = today.getMonth();
            let targetMonthYear;
            if (feeStructure.feeType === 'Monthly Fee') {
                targetMonthYear = `${currentYear}-${String(currentMonth + 1).padStart(2, '0')}`;
            } else if (feeStructure.feeType === 'Yearly Fee') {
                targetMonthYear = `${currentYear}-01`;
            }
            if (!targetMonthYear) {
                return;
            }
            try {
                const studentsInClass = await getAll(STORE_STUDENTS, 'classId', feeStructure.classId);
                const dueStore = await getObjectStore(STORE_MONTHLY_DUES, 'readwrite');
                for (const student of studentsInClass) {
                    const index = dueStore.index('studentId_feeType_monthYear');
                    const checkReq = index.get([student.id, feeStructure.feeType, targetMonthYear]);
                    const existingDue = await new Promise((resolve, reject) => {
                        checkReq.onsuccess = () => resolve(checkReq.result);
                        checkReq.onerror = (e) => reject(e.target.error);
                    });
                    if (existingDue) {
                        if (existingDue.amountDue !== feeStructure.amount) {
                            existingDue.amountDue = feeStructure.amount;
                            if (existingDue.amountPaid + existingDue.discount >= existingDue.amountDue) {
                                existingDue.status = 'paid';
                            } else if (existingDue.amountPaid > 0 || existingDue.discount > 0) {
                                existingDue.status = 'partial';
                            } else {
                                existingDue.status = 'due';
                            }
                            await new Promise((resolve, reject) => {
                                const putReq = dueStore.put(existingDue);
                        ...
                        await new Promise((resolve, reject) => {
                            const addReq = dueStore.add(newDue);
                            addReq.onsuccess = resolve;
                            addReq.onerror = (e) => reject(e.target.error);
                        });
                    }
                ...
                console.error("Failed to update/add monthly dues after fee structure change:", error);
            }
        }
        async function printSalarySlip(expenseId) {
            const slipContainer = document.getElementById('salarySlipContainer');
            if (!slipContainer) {
                console.error('Salary slip container not found.');
                return;
            }
            try {
                const payment = await getById(STORE_EXPENSES, expenseId);
                if (!payment) {
                    alert('Payment record not found.');
                    return;
                }
                const schoolSetting = await getById(STORE_SETTINGS, 'schoolName');
                const schoolName = schoolSetting ? schoolSetting.value : 'Your Madrasa Name';
                document.getElementById('slipSchoolName').textContent = schoolName;
                document.getElementById('slipId').textContent = `S-${payment.id}`;
                document.getElementById('slipPaymentDate').textContent = payment.date;
                document.getElementById('slipTeacherName').textContent = payment.paidTo;
                document.getElementById('slipMonth').textContent = payment.description.replace('Salary for ', '');
                document.getElementById('slipAmountPaid').textContent = `Rs. ${payment.amount.toLocaleString()}`;
                document.getElementById('slipNotes').textContent = payment.notes || '-';
                slipContainer.querySelector('.generated-on').textContent = `Generated: ${new Date().toLocaleString()}`;
                ...
                const link = document.createElement('a');
                link.href = canvas.toDataURL('image/png');
                link.download = `SalarySlip_${payment.paidTo.replace(/\s/g, '_')}_${payment.date}.png`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                ...
                alert('Failed to export salary slip as PNG.');
                slipContainer.style.display = 'none';
            }
        }
...
/* Script Block 5 */
        ...
        let studentsByClassChartInstance = null;
        let incomeVsExpensesChartInstance = null;
        let attendanceSummaryChartInstance = null;
        let averageMarksChartInstance = null;
        let feeStatusChartInstance = null;
        let salaryDistributionChartInstance = null;
        async function renderDashboardCharts() {
            if (studentsByClassChartInstance) studentsByClassChartInstance.destroy();
            if (incomeVsExpensesChartInstance) incomeVsExpensesChartInstance.destroy();
            if (attendanceSummaryChartInstance) attendanceSummaryChartInstance.destroy();
            if (averageMarksChartInstance) averageMarksChartInstance.destroy();
            if (feeStatusChartInstance) feeStatusChartInstance.destroy();
            if (salaryDistributionChartInstance) salaryDistributionChartInstance.destroy();
            ...
            try {
                ...
                const studentCountByClass = students.reduce((acc, student) => {
                    const className = classMap[student.classId] || 'نامعلوم';
                    acc[className] = (acc[className] || 0) + 1;
                    return acc;
                }, {});
                const labels = Object.keys(studentCountByClass);
                const data = Object.values(studentCountByClass);
                const backgroundColors = labels.map((_, i) => `hsl(${(i * 360 / labels.length) % 360}, 70%, 70%)`);
                studentsByClassChartInstance = new Chart(studentsByClassCtx, {
                    type: 'pie',
                    data: {
                        labels: labels,
                        datasets: [{
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: {
                                ...
                                labels: {
                                    font: {
                                        family: 'Noto Nastaliq Urdu, sans-serif'
                                    }
                                }
                            },
                            title: {
                                display: true,
                                text: 'کلاس کے لحاظ سے طلباء کی تقسیم',
                                font: {
                                    family: 'Noto Nastaliq Urdu, sans-serif'
                                }
                            }
                ...
                console.error("Error rendering Students by Class Chart:", error);
                studentsByClassCtx.clearRect(0, 0, studentsByClassCtx.canvas.width, studentsByClassCtx.canvas.height);
                studentsByClassCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
            }
            ...
            try {
                ...
                const recentAttendance = attendanceRecords.filter(rec => {
                    const recDate = new Date(rec.date);
                    return recDate >= thirtyDaysAgo && recDate.toISOString().split('T')[0] <= todayStr;
                });
                const statusCounts = recentAttendance.reduce((acc, rec) => {
                    acc[rec.status] = (acc[rec.status] || 0) + 1;
                    return acc;
                }, {});
                const labels = ['حاضر', 'غیر حاضر', 'رخصت'];
                const data = [
                    statusCounts['Present'] || 0,
                    statusCounts['Absent'] || 0,
                    statusCounts['Leave'] || 0
                ];
                const backgroundColors = ['#28a745', '#dc3545', '#ffc107'];
                attendanceSummaryChartInstance = new Chart(attendanceSummaryCtx, {
                    type: 'doughnut',
                    data: {
                        labels: labels,
                        datasets: [{
                ...
                attendanceSummaryCtx.clearRect(0, 0, attendanceSummaryCtx.canvas.width, attendanceSummaryCtx.canvas.height);
                attendanceSummaryCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
            }
            ...
            try {
                ...
                const classes = await getAll(STORE_CLASSES);
                const classMap = classes.reduce((acc, cls) => { acc[cls.id] = cls.name; return acc; }, {});
                const marksByClass = results.reduce((acc, res) => {
                    const className = classMap[res.classId] || 'نامعلوم';
                    if (!acc[className]) {
                        acc[className] = { totalObtained: 0, totalMax: 0 };
                    }
                    acc[className].totalObtained += res.obtainedMarks;
                    acc[className].totalMax += res.totalMarks;
                    return acc;
                }, {});
                const labels = Object.keys(marksByClass);
                const data = labels.map(label => {
                    const classData = marksByClass[label];
                    return classData.totalMax > 0 ? ((classData.totalObtained / classData.totalMax) * 100).toFixed(1) : 0;
                });
                const backgroundColors = labels.map((_, i) => `hsl(${(i * 50) % 360}, 60%, 65%)`);
                averageMarksChartInstance = new Chart(averageMarksCtx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [{
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'کلاس کے لحاظ سے اوسط امتحانی نمبر',
                                font: {
                                    family: 'Noto Nastaliq Urdu, sans-serif'
                                }
                            }
                ...
                console.error("Error rendering Average Marks Chart:", error);
                averageMarksCtx.clearRect(0, 0, averageMarksCtx.canvas.width, averageMarksCtx.canvas.height);
                averageMarksCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
            }
            const feeStatusCtx = document.getElementById('feeStatusChart').getContext('2d');
            const totalExpectedFeesDisplay = document.getElementById('totalExpectedFeesDisplay');
            const totalPaidFeesDisplay = document.getElementById('totalPaidFeesDisplay');
            const feeStatusChartFilter = document.getElementById('feeStatusChartFilter');
            try {
                const allMonthlyDues = await getAll(STORE_MONTHLY_DUES);
                let filteredMonthlyDues = [];
                const filterType = feeStatusChartFilter.value;
                const today = new Date();
                let startDate = null;
                let endDate = null;
                ...
                filteredMonthlyDues = allMonthlyDues.filter(due => {
                    if (filterType === 'overall') return true;
                    const [dueYear, dueMonth] = due.monthYear.split('-').map(Number);
                    const dueStartDate = new Date(dueYear, dueMonth - 1, 1);
                    const dueEndDate = new Date(dueYear, dueMonth, 0);
                    return (dueEndDate >= startDate && dueStartDate <= endDate);
                });
                let totalExpectedOverall = 0;
                let totalPaidOverall = 0;
                filteredMonthlyDues.forEach(due => {
                    const currentExpectedAmount = due.amountDue || due.feeAmount || 0;
                    const currentPaidAmount = due.amountPaid || 0;
                    totalExpectedOverall += currentExpectedAmount;
                    totalPaidOverall += currentPaidAmount;
                });
                const remainingDueOverall = totalExpectedOverall - totalPaidOverall;
                const labels = ['ادا شدہ فیس', 'واجب الادا فیس'];
                const data = [
                    totalPaidOverall,
                    remainingDueOverall > 0 ? remainingDueOverall : 0
                ];
                const backgroundColors = ['#17a2b8', '#ff4d4d'];
                feeStatusChartInstance = new Chart(feeStatusCtx, {
                    type: 'doughnut',
                    data: {
                        labels: labels,
                        datasets: [{
                ...
                if (totalExpectedFeesDisplay) {
                    totalExpectedFeesDisplay.textContent = totalExpectedOverall.toLocaleString();
                }
                if (totalPaidFeesDisplay) {
                    totalPaidFeesDisplay.textContent = totalPaidOverall.toLocaleString();
                }
            } catch (error) {
                console.error("Error rendering Fee Status Chart:", error);
                feeStatusCtx.clearRect(0, 0, feeStatusCtx.canvas.width, feeStatusCtx.canvas.height);
                feeStatusCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
                if (totalExpectedFeesDisplay) {
                    totalExpectedFeesDisplay.textContent = 'خرابی';
                }
                if (totalPaidFeesDisplay) {
                    totalPaidFeesDisplay.textContent = 'خرابی';
                }
            }
            ...
            try {
                ...
                incomeVsExpensesCtx.clearRect(0, 0, incomeVsExpensesCtx.canvas.width, incomeVsExpensesCtx.canvas.height);
                incomeVsExpensesCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
            }
            const salaryDistributionCtx = document.getElementById('salaryDistributionChart').getContext('2d');
            try {
                const allExpenses = await getAll(STORE_EXPENSES);
                const teachers = await getAll(STORE_TEACHERS);
                const teacherNamesMap = teachers.reduce((acc, t) => { acc[t.name] = t.name; return acc; }, {});
                const oneYearAgo = new Date();
                oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
                const todayStr = new Date().toISOString().split('T')[0];
                const recentSalaryPayments = allExpenses.filter(exp => {
                    const expDate = new Date(exp.date);
                    return exp.recordType === 'expense' &&
                        exp.category === 'Salary' &&
                        exp.paidTo &&
                        teacherNamesMap[exp.paidTo] &&
                        expDate >= oneYearAgo && expDate.toISOString().split('T')[0] <= todayStr;
                });
                const salaryByTeacher = recentSalaryPayments.reduce((acc, exp) => {
                    acc[exp.paidTo] = (acc[exp.paidTo] || 0) + exp.amount;
                    return acc;
                }, {});
                const labels = Object.keys(salaryByTeacher);
                const data = Object.values(salaryByTeacher);
                const backgroundColors = labels.map((_, i) => `hsl(${(i * 70) % 360}, 65%, 60%)`);
                salaryDistributionChartInstance = new Chart(salaryDistributionCtx, {
                    type: 'bar',
                    data: {
                        labels: labels,
                        datasets: [{
                            label: 'کل ادا شدہ تنخواہ (آخری 12 ماہ)',
                            data: data,
                            backgroundColor: backgroundColors,
                            borderColor: 'rgba(0,0,0,0.1)',
                            borderWidth: 1
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'اساتذہ کے لحاظ سے تنخواہ کی تقسیم',
                                font: {
                                    family: 'Noto Nastaliq Urdu, sans-serif'
                                }
                            }
                ...
                salaryDistributionCtx.clearRect(0, 0, salaryDistributionCtx.canvas.width, salaryDistributionCtx.canvas.height);
                salaryDistributionCtx.fillText('چارٹ لوڈ نہیں ہو سکا۔', 10, 50);
            }
        }
