...
define('DB_NAME', 'school_madrasa_dba');
...
function create_tables($mysqli)
{
    $tables = [
        "CREATE TABLE IF NOT EXISTS settings (
            ...
            currentSession VARCHAR(50),
            sessionStart DATE,
            sessionEnd DATE,
            schoolTimeStart TIME,
            schoolTimeEnd TIME,
            periodDuration INT,
            weeklyHoliday TEXT,
            feeDueDay INT,
            feeLateFine DECIMAL(10, 2),
            libraryReturnDays INT,
            libraryLateFine DECIMAL(10, 2),
            primaryColor VARCHAR(7),
            secondaryColor VARCHAR(7),
            fontSize VARCHAR(20),
            fontFamily VARCHAR(255),
            sidebarPosition VARCHAR(10),
            sidebarTheme VARCHAR(10),
            language VARCHAR(10),
            autoBackupEnabled BOOLEAN DEFAULT FALSE,
            autoBackupFrequency VARCHAR(20),
        ...
        "CREATE TABLE IF NOT EXISTS fees (
            ...
            total DECIMAL(10, 2),
            monthlyFeeAtTimeOfPayment DECIMAL(10,2),
            date DATE NOT NULL,
        ...
        "CREATE TABLE IF NOT EXISTS salary (
            ...
            deductions DECIMAL(10, 2),
            total DECIMAL(10, 2),
            grossSalaryAtTimeOfPayment DECIMAL(10,2),
            date DATE NOT NULL,
            paymentMethod VARCHAR(50),
...
if (!create_tables($mysqli)) {
    die("Database setup failed. Check error logs.");
}
...
if (isset($_POST['action']) || isset($_GET['action'])) {
    ...
    switch ($action) {
        case 'add_data':
        case 'update_data':
            handle_add_update_data($mysqli);
            break;
        case 'get_all':
            handle_get_all($mysqli);
            break;
        case 'get_one':
            handle_get_one($mysqli);
            break;
        case 'delete_data':
            handle_delete_data($mysqli);
            break;
        case 'get_count':
            handle_get_count($mysqli);
            break;
        case 'get_by_index':
            handle_get_by_index($mysqli);
            break;
        case 'login':
            handle_login($mysqli);
            break;
        case 'fetch_settings':
            handle_fetch_settings($mysqli);
            break;
        case 'fetch_fee_structure_for_student':
            handle_fetch_fee_structure_for_student($mysqli);
            break;
        case 'create_backup_php':
            handle_create_backup_php($mysqli);
            break;
        case 'restore_backup_php':
            handle_restore_backup_php($mysqli);
            break;
        case 'get_filtered_students':
            handle_get_filtered_students($mysqli);
            break;
        case 'get_filtered_teachers':
            handle_get_filtered_teachers($mysqli);
            break;
        case 'get_filtered_staff':
            handle_get_filtered_staff($mysqli);
            break;
        default:
...
function handle_add_update_data($mysqli)
{
    ...
    if ($storeName === 'teachers' && isset($data['subjects'])) {
        $data['subjects'] = json_encode($data['subjects']);
    }
    if ($storeName === 'teachers' && isset($data['classes'])) {
        $data['classes'] = json_encode($data['classes']);
    }
    if ($storeName === 'exams' && isset($data['classes'])) {
        $data['classes'] = json_encode($data['classes']);
    }
    if ($storeName === 'settings' && isset($data['weeklyHoliday'])) {
    ...
    if ($id && $_POST['action'] === 'update_data') {
        if ($storeName === 'settings') {
            ...
            $types = "";
            foreach ($data as $key => $value) {
                if (!in_array($key, $excluded_fields) && $key !== 'id') {
                    $update_fields[] = "`{$key}` = ?";
                    $params[] = $value;
                    $types .= get_param_type($value);
                }
...
function handle_get_all($mysqli)
{
    ...
    if ($result) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (isset($row['classes']) && $storeName === 'teachers') { // Teachers store
                $row['classes'] = json_decode($row['classes'], true);
            }
            if (isset($row['classes']) && $storeName === 'exams') { // Exams store
                $row['classes'] = json_decode($row['classes'], true);
            }
            if (isset($row['weeklyHoliday'])) {
                $row['weeklyHoliday'] = json_decode($row['weeklyHoliday'], true);
            }
            $data[] = $row;
        }
...
function handle_get_one($mysqli)
{
    $storeName = $_GET['store'];
    $id = $_GET['id'];
    $sql = "SELECT * FROM `{$storeName}` WHERE `id` = ?";
    $stmt = $mysqli->prepare($sql);
    if ($stmt === false) {
        echo json_encode(['status' => 'error', 'message' => 'Prepare failed: ' . $mysqli->error]);
        return;
    ...
    if ($result->num_rows > 0) {
        ...
        if (isset($row['classes']) && $storeName === 'teachers') {
            $row['classes'] = json_decode($row['classes'], true);
        }
        if (isset($row['classes']) && $storeName === 'exams') {
            $row['classes'] = json_decode($row['classes'], true);
        }
        if (isset($row['weeklyHoliday'])) {
            $row['weeklyHoliday'] = json_decode($row['weeklyHoliday'], true);
...
function handle_delete_data($mysqli)
{
    $storeName = $_POST['store'];
    $id = $_POST['id'];
    $sql = "DELETE FROM `{$storeName}` WHERE `id` = ?";
    $stmt = $mysqli->prepare($sql);
    if ($stmt === false) {
        echo json_encode(['status' => 'error', 'message' => 'Prepare failed: ' . $mysqli->error]);
        return;
...
function handle_get_count($mysqli)
{
    $storeName = $_GET['store'];
    $sql = "SELECT COUNT(*) as count FROM `{$storeName}`";
    $result = $mysqli->query($sql);
    if ($result) {
        $row = $result->fetch_assoc();
...
function handle_get_by_index($mysqli)
{
    $storeName = $_GET['store'];
    $indexName = $_GET['indexName'];
    $value = $_GET['value'];
    $multipleValues = isset($_GET['multipleValues']) && $_GET['multipleValues'] === 'true';

    $sql = "SELECT * FROM `{$storeName}` WHERE `{$indexName}` = ?";
    $types = "s"; // Default to string for now

    if ($multipleValues) {
        $values_array = json_decode($value, true);
        if (!is_array($values_array) || empty($values_array)) {
            echo json_encode(['status' => 'error', 'message' => 'Invalid multiple values provided.']);
            return;
        }
        $placeholders = implode(',', array_fill(0, count($values_array), '?'));
        $sql = "SELECT * FROM `{$storeName}` WHERE `{$indexName}` IN ({$placeholders})";
        $types = str_repeat('s', count($values_array));
        $params = $values_array;
    } else {
        $params = [$value];
        ...
        if (is_numeric($value)) {
            ...
            $types = "i"; // Booleans stored as 0 or 1
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (isset($row['classes']) && ($storeName === 'teachers' || $storeName === 'exams')) {
            $row['classes'] = json_decode($row['classes'], true);
        }
        if (isset($row['weeklyHoliday'])) {
            $row['weeklyHoliday'] = json_decode($row['weeklyHoliday'], true);
...
{
    $sql = "SELECT * FROM settings WHERE id = 'general' LIMIT 1";
    $result = $mysqli->query($sql);

    if ($result && $result->num_rows > 0) {
        $settings = $result->fetch_assoc();
        if (isset($settings['weeklyHoliday'])) {
            $settings['weeklyHoliday'] = json_decode($settings['weeklyHoliday'], true);
        }
        echo json_encode(['status' => 'success', 'settings' => $settings]);
    } else {
        echo json_encode(['status' => 'success', 'settings' => null]); // No settings found, send null
    }
}

function handle_fetch_fee_structure_for_student($mysqli)
{
    $classId = $_GET['classId'];
    $feeCategory = $_GET['feeCategory'];

    $sql = "SELECT * FROM fee_structure WHERE classId = ? AND category = ?";
    $stmt = $mysqli->prepare($sql);
    if ($stmt === false) {
        echo json_encode(['status' => 'error', 'message' => 'Prepare failed: ' . $mysqli->error]);
        return;
    }
    $stmt->bind_param("is", $classId, $feeCategory);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $structure = $result->fetch_assoc();
        echo json_encode(['status' => 'success', 'structure' => $structure]);
    } else {
        // Fallback to 'General' category if specific category not found
        $sql_general = "SELECT * FROM fee_structure WHERE classId = ? AND category = 'General'";
        $stmt_general = $mysqli->prepare($sql_general);
        if ($stmt_general === false) {
            echo json_encode(['status' => 'error', 'message' => 'Prepare fallback failed: ' . $mysqli->error]);
            return;
        }
        $stmt_general->bind_param("i", $classId);
        $stmt_general->execute();
        $result_general = $stmt_general->get_result();
        if ($result_general->num_rows > 0) {
            $structure_general = $result_general->fetch_assoc();
            echo json_encode(['status' => 'success', 'structure' => $structure_general]);
        } else {
            echo json_encode(['status' => 'success', 'structure' => null]);
        }
        $stmt_general->close();
    }
    $stmt->close();
...
{
    $classIds = isset($_GET['classIds']) ? json_decode($_GET['classIds'], true) : [];
    $sectionIds = isset($_GET['sectionIds']) ? json_decode($_GET['sectionIds'], true) : [];
    $gender = $_GET['gender'] ?? '';
    $feeCategory = $_GET['feeCategory'] ?? '';
    $admissionDateFrom = $_GET['admissionDateFrom'] ?? '';
    $admissionDateTo = $_GET['admissionDateTo'] ?? '';
    $status = $_GET['status'] ?? 'active'; // Default to active

    $sql = "SELECT s.*, c.name as className, sec.name as sectionName FROM students s
    ...
    if (!empty($classIds)) {
        ...
        $sql .= " AND s.classId IN ({$placeholders})";
    ...
    if (!empty($sectionIds)) {
        ...
        $sql .= " AND s.sectionId IN ({$placeholders})";
    ...
    if (!empty($feeCategory)) {
        $sql .= " AND s.feeCategory = ?";
        $params[] = $feeCategory;
        $types .= "s";
    }
    if (!empty($admissionDateFrom)) {
        $sql .= " AND s.admissionDate >= ?";
        $params[] = $admissionDateFrom;
        $types .= "s";
    }
    if (!empty($admissionDateTo)) {
        $sql .= " AND s.admissionDate <= ?";
        $params[] = $admissionDateTo;
        $types .= "s";
    }
    if (!empty($status)) {
        $sql .= " AND s.status = ?";
...
function handle_get_filtered_teachers($mysqli)
{
    $subjectIds = isset($_GET['subjectIds']) ? json_decode($_GET['subjectIds'], true) : [];
    $qualificationSearch = $_GET['qualificationSearch'] ?? '';
    $joiningDateFrom = $_GET['joiningDateFrom'] ?? '';
    $joiningDateTo = $_GET['joiningDateTo'] ?? '';
    ...
    if (!empty($subjectIds)) {
        // This is a bit tricky with JSON_CONTAINS in WHERE clause, might be better to fetch all and filter in PHP
        // For simplicity, we'll iterate through all and filter in PHP.
        // Or you can try: WHERE JSON_CONTAINS(subjects, ?)
        // But for multiple values, it gets complex. For now, fetch all and filter client-side or server-side after fetch.
    }
    if (!empty($qualificationSearch)) {
        $sql .= " AND qualification LIKE ?";
        $params[] = "%" . $qualificationSearch . "%";
        $types .= "s";
    }
    ...
    while ($row = $result->fetch_assoc()) {
        if (isset($row['subjects'])) {
            $row['subjects'] = json_decode($row['subjects'], true); // Decode subjects string back to array
        }
        if (isset($row['classes'])) {
            $row['classes'] = json_decode($row['classes'], true); // Decode classes string back to array
        }
        $data[] = $row;
    }

    // Apply subject filter after fetching all if subjectIds are provided
    if (!empty($subjectIds)) {
        $data = array_filter($data, function ($teacher) use ($subjectIds) {
            if (!isset($teacher['subjects']) || !is_array($teacher['subjects'])) {
                return false;
            }
            foreach ($subjectIds as $subId) {
                if (in_array(strval($subId), $teacher['subjects'])) { // Ensure string comparison if needed
                    return true;
                }
            }
            return false;
...
function handle_get_filtered_staff($mysqli)
{
...
function get_param_type($var)
{
    ...
    if (is_bool($var)) return "i"; // Booleans typically stored as tinyint(1)
    if (is_null($var)) return "s"; // Treat null as string for binding purposes, SQL will handle NULL value correctly.
...
<head>
    ...
    <meta name="description" content="Comprehensive school and madrasa management system for student, teacher, staff, fees, and attendance." />
    <meta name="keywords" content="school management, madrasa management, student information system, fees, attendance, Pakistan, education software" />
...
<body>
    ...
    <div class="modal" id="send-notice-modal">
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div id="notice-student-info"
                style="margin-bottom: 15px; background: #f9f9f9; padding: 10px; border-radius: 4px;">
            </div>
        ...
        <div class="section-view" id="students-view">
            ...
            <div class="tab-content active" id="students-list">
                <div class="form-container">
                    <div class="form-row">
                        <div class="form-group">
                            <input class="form-control" id="student-search"
                                placeholder="Search by student name, roll number or class" type="text" />
                        </div>
        ...
        <div class="section-view" id="teachers-view">
            ...
            <div class="tab-content active" id="teachers-list">
                <div class="form-container">
                    <div class="form-row">
                        <div class="form-group">
                            <input class="form-control" id="teacher-search"
                                placeholder="Search by teacher name, subject or class" type="text" />
                        </div>
                        <div class="form-group" style="flex: 0 0 auto;">
        ...
        <div class="section-view" id="staff-view">
            ...
            <div class="tab-content active" id="staff-list">
                <div class="form-container">
                    <div class="form-row">
                        <div class="form-group">
                            <input class="form-control" id="staff-search"
                                placeholder="Search by staff name, designation or department" type="text" />
                        </div>
                        <div class="form-group" style="flex: 0 0 auto;">
        ...
        <div class="section-view" id="attendance-view">
                ...
                <div class="table-container" id="view-attendance-table-container" style="display: none;">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary"
                            onclick="exportElementAsPNG('view-attendance-table-container', 'AttendanceView.png')"
                            type="button">Export
                            PNG</button>
                    </div>
                </div>
            ...
            <div class="tab-content" id="attendance-report">
                ...
                <div class="table-container" id="report-attendance-table-container" style="display: none;">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary"
                            onclick="exportElementAsPNG('report-attendance-table-container', 'AttendanceReport.png')"
                            type="button">Export
                            PNG</button>
                        <button class="btn btn-success" id="export-attendance-report" style="margin-left: 10px;"
                    ...
                    <form id="exam-form">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" id="exam-passing-percent" max="100" min="0" required=""
                ...
                <div class="form-container">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary" id="generate-result-cards" type="button">Generate Result
                            Cards</button>
                        <button class="btn btn-success" id="export-all-result-cards" style="margin-left: 10px;"
                            type="button">Export All as ZIP (PNG)</button>
                    </div>
                </div>
        ...
        <div class="section-view" id="fees-view">
                ...
                <div class="form-container">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input class="form-control" id="fee-student-search" placeholder="Search by Roll No. or Name"
                                type="text" />
                            <div id="fee-search-results"
                                style="position: absolute; background-color: white; border: 1px solid #ddd; z-index: 100; width: calc(100% - 20px); max-height: 200px; overflow-y: auto; display: none;">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <form id="fee-form">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input class="form-control" id="fee-balance" readonly=""
                                    style="font-weight:bold; color:#c0392b;" type="number" />
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-row">
                            ...
                            <button class="btn btn-success" id="print-receipt" style="margin-left: 10px;"
            ...
            <div class="tab-content" id="fee-reports">
                ...
                <div class="table-container" id="fee-report-table-container" style="display: none;">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary"
                            onclick="exportElementAsPNG('fee-report-table-container', 'FeeReport.png')"
                            type="button">Export
                            PNG</button>
                        <button class="btn btn-success" id="export-fee-report" style="margin-left: 10px;"
                            type="button">Export</button>
                    </div>
                </div>
            </div>
            <div class="tab-content" id="fee-defaulters">
                ...
                <div class="table-container" id="defaulters-table-container" style="display: none;">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary"
                            onclick="exportElementAsPNG('defaulters-table-container', 'FeeDefaulters.png')"
                            type="button">Export
                            PNG</button>
                        <button class="btn btn-success" id="send-reminders" style="margin-left: 10px;"
                            type="button">Send Reminders</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="section-view" id="salaries-view">
                ...
                <div class="form-container">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" id="salary-employee-search"
                                placeholder="Search by Name or ID No." type="text" />
                        </div>
                    </div>
                </div>
                ...
                <div class="form-container" id="salary-payment-form" style="display: none;">
                    ...
                    <form id="salary-form">
                        ...
                        <div class="form-row">
                            ...
                            <button class="btn btn-success" id="print-salary-slip" style="margin-left: 10px;"
                                type="button">Print Salary Slip</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="tab-content" id="salary-reports">
                ...
                <div class="table-container" id="salary-report-table-container" style="display: none;">
                    ...
                    <div class="form-row">
                        <button class="btn btn-primary"
                            onclick="exportElementAsPNG('salary-report-table-container', 'SalaryReport.png')"
                            type="button">Export
                            PNG</button>
                        <button class="btn btn-success" id="export-salary-report" style="margin-left: 10px;"
                            type="button">Export</button>
                    </div>
                </div>
            </div>
            ...
            <div class="form-container no-print">
                ...
                <div class="form-row">
                    <div class="form-group" style="flex: 0 0 auto; align-items: center; display: flex;">
                        <input id="auto-backup-enable" style="width: 20px; height: 20px; margin-right: 10px;"
                            type="checkbox" />
        ...
        <div class="section-view" id="library-view">
            ...
            <div class="tab-content" id="add-book">
                <div class="form-container">
                    ...
                    <form id="book-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Book Name</label><input
                                    class="form-control" id="book-name" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">Author</label><input class="form-control"
                                    id="book-author" required="" type="text" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Category</label><input
                                    class="form-control" id="book-category" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">ISBN</label><input class="form-control"
                                    id="book-isbn" type="text" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Copies</label><input class="form-control"
                                    id="book-copies" min="1" required="" type="number" /></div>
                            <div class="form-group"><label class="form-label">Shelf Location</label><input
                                    class="form-control" id="book-shelf" type="text" /></div>
                        </div>
                        <div class="form-row">
        ...
        <div class="section-view" id="users-view">
                    ...
                    <form id="user-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Name</label><input class="form-control"
                                    id="user-name" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">Username</label><input
                                    class="form-control" id="user-username" required="" type="text" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Password</label><input
                                    class="form-control" id="user-password" required="" type="password" /></div>
                            <div class="form-group"><label class="form-label">Confirm Password</label><input
                                    class="form-control" id="user-confirm-password" required="" type="password" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Email</label><input class="form-control"
                                    id="user-email" required="" type="email" /></div>
                            <div class="form-group"><label class="form-label">Mobile</label><input class="form-control"
                                    id="user-mobile" type="tel" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
            ...
            <div class="tab-content" id="user-roles">
                <div class="form-container">
                    ...
                    <form id="role-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Role Name</label><input
                                    class="form-control" id="role-name" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">Description</label><input
                                    class="form-control" id="role-description" type="text" /></div>
                        </div>
                        <div class="form-row" id="permissions-container"
                            style="flex-direction: column; align-items: start; gap: 5px;">
                        </div>
            ...
            <div class="tab-content active" id="general-settings">
                <div class="form-container">
                    ...
                    <form id="school-settings-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">School Name</label><input
                                    class="form-control" id="school-name" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">Principal Name</label><input
                                    class="form-control" id="principal-name" required="" type="text" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Address</label><textarea
                                    class="form-control" id="school-address" required="" rows="3"></textarea></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Contact No.</label><input
                                    class="form-control" id="school-contact" required="" type="tel" /></div>
                            <div class="form-group"><label class="form-label">Email</label><input class="form-control"
                                    id="school-email" type="email" /></div>
                        </div>
                        <div class="form-row">
            ...
            <div class="tab-content" id="academic-settings">
                <div class="form-container">
                    ...
                    <form id="academic-settings-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Current Session</label><input
                                    class="form-control" id="setting-current-session" required="" type="text" /></div>
                            <div class="form-group"><label class="form-label">Session Start Date</label><input
                                    class="form-control" id="setting-session-start" required="" type="date" /></div>
                            <div class="form-group"><label class="form-label">Session End Date</label><input
                                    class="form-control" id="setting-session-end" required="" type="date" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">School Time Start</label><input
                                    class="form-control" id="setting-school-time-start" type="time" /></div>
                            <div class="form-group"><label class="form-label">School Time End</label><input
                                    class="form-control" id="setting-school-time-end" type="time" /></div>
                            <div class="form-group"><label class="form-label">Period Duration (minutes)</label><input
                                    class="form-control" id="setting-period-duration" type="number" /></div>
                        </div>
                        <div class="form-row">
                            ...
                            <div class="form-group"><label class="form-label">Fee Due Day of Month</label><input
                                    class="form-control" id="setting-fee-due-day" type="number" /></div>
                            <div class="form-group"><label class="form-label">Fee Late Fine per Day</label><input
                                    class="form-control" id="setting-fee-late-fine" type="number" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Library Return Days</label><input
                                    class="form-control" id="setting-library-return-days" type="number" /></div>
                            <div class="form-group"><label class="form-label">Library Late Fine per Day</label><input
                                    class="form-control" id="setting-library-late-fine" type="number" /></div>
                        </div>
            ...
            <div class="tab-content" id="theme-settings">
                <div class="form-container">
                    ...
                    <form id="theme-settings-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Primary Color</label><input
                                    class="form-control" id="setting-primary-color" type="color" /></div>
                            <div class="form-group"><label class="form-label">Secondary Color</label><input
                                    class="form-control" id="setting-secondary-color" type="color" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Font Size</label><select
                                    class="form-control" id="setting-font-size">
                                    <option value="small">Small</option>
                                    <option value="medium">Medium</option>
                                    <option value="large">Large</option>
                                </select></div>
                            <div class="form-group"><label class="form-label">Font Family</label><input
                                    class="form-control" id="setting-font-family" type="text" /></div>
                        </div>
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Sidebar Position</label><select
                                    class="form-control" id="setting-sidebar-position">
                                    <option value="left">Left</option>
                                    <option value="right">Right</option>
                                </select></div>
                            <div class="form-group"><label class="form-label">Sidebar Theme</label><select
                                    class="form-control" id="setting-sidebar-theme">
                        ...
                        <div class="form-row">
                            <button class="btn btn-primary" type="submit">Save Settings</button>
                            <button class="btn btn-danger" id="reset-theme-btn" style="margin-left: 10px;"
                                type="button">Reset Theme</button>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <form id="system-settings-form">
                        <div class="form-row">
                            <div class="form-group"><label class="form-label">Language</label><select
                                    class="form-control" id="system-language">
                ...
                <div class="form-container no-print">
                    ...
                    <div class="form-row">
                        ...
                        <button class="btn btn-success"
                            onclick="exportElementAsPNG('id-card-output', 'All_ID_Cards_View.png')"
                            style="margin-left:10px;">Export View as PNG</button>
                        <button class="btn btn-info" id="bulk-export-zip-btn" style="margin-left:10px;">Export All to
                            ZIP</button>
                    </div>
                </div>
                <div class="dashboard-cards" id="id-card-output"
                    style="background: #e9ecef; padding: 20px; justify-content: center;"></div>
            </div>
                ...
                <div class="form-container no-print">
                    ...
                    <div class="form-row" id="achievement-details-row" style="display:none;">
                        <div class="form-group">
                            ...
                            <input class="form-control" id="achievement-description"
                                placeholder="e.g. First position in speech competition" type="text" />
                        </div>
                    </div>
                    <div class="form-row">
                        <button class="btn btn-primary" id="generate-certificate-btn">Generate Certificate</button>
                        <button class="btn btn-success"
                            onclick="exportElementAsPNG('certificate-output', 'Certificate_View.png')"
                            style="margin-left:10px;">Export View as PNG</button>
                    </div>
                </div>
    ...
    <script>
        ...
        const STORES = {
            ...
            BACKUP: 'backups' // Changed to 'backups' to match PHP table name
        };
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('school-settings-form').addEventListener('submit', async function(e) {
                ...
                await addToStore(STORES.SETTINGS, updated, 'general');
                showNotification('Settings saved successfully', 'success');
            });
            document.getElementById('system-settings-form').addEventListener('submit', async function(e) {
                e.preventDefault();
                const existingSettings = await getFromStore(STORES.SETTINGS, 'general');
                const updatedSettings = {
                    ...existingSettings, // Merge existing settings
                    language: document.getElementById('system-language').value,
                };
                await addToStore(STORES.SETTINGS, updatedSettings, 'general'); // Update 'general' settings
        ...
        async function getFromStore(storeName, id) {
            const response = await fetch(`indexnewonexact.php?action=get_one&store=${storeName}&id=${id}`);
        ...
        async function getAllFromStore(storeName, limit = null) {
            let url = `indexnewonexact.php?action=get_all&store=${storeName}`;
            if (limit) {
                url += `&limit=${limit}`;
            }
        ...
        async function getAllFromIndex(storeName, indexName, value) {
            let url = `indexnewonexact.php?action=get_by_index&store=${storeName}&indexName=${indexName}`;
            if (Array.isArray(value)) {
                url += `&value=${encodeURIComponent(JSON.stringify(value))}&multipleValues=true`;
            } else {
        ...
        async function addToStore(storeName, data, upsertId = null) {
            const formData = new FormData();
            formData.append('action', upsertId ? 'update_data' : 'add_data');
            formData.append('store', storeName);
            formData.append('data', JSON.stringify(data));
            if (upsertId) {
                formData.append('id', upsertId);
            }
        ...
        async function updateInStore(storeName, id, data) {
            const formData = new FormData();
        ...
        async function getCount(storeName) {
            const response = await fetch(`indexnewonexact.php?action=get_count&store=${storeName}`);
            const data = await response.json();
            if (data.status === 'success') {
                return data.count;
            }
        ...
        async function loadSchoolSettings() {
            try {
                const response = await fetch('indexnewonexact.php?action=fetch_settings');
                const data = await response.json();
        ...
        async function showFeeReceipt(feeData, studentData) {
            ...
            const monthName = new Date(feeData.year, feeData.month - 1, 1).toLocaleString('en-US', {
                month: 'long'
            });
            const feeStructure = await getFeeStructureForStudent(null, studentData.classId, studentData.feeCategory);
            const fullMonthlyFee = feeStructure?.monthlyFee || studentData.monthlyFee || 0;
            const allPaidRecordsForMonth = (await getAllFromIndex(STORES.FEES, 'studentId', feeData.studentId)).filter(rec => rec.month === feeData.month && rec.year === feeData.year);
            const totalPaidForMonth = allPaidRecordsForMonth.reduce((sum, rec) => sum + (parseFloat(rec.amount) - parseFloat(rec.discount)), 0);
            const totalFinePaid = allPaidRecordsForMonth.reduce((sum, rec) => sum + parseFloat(rec.fine), 0);
        ...
        async function loadDashboardData() {
            try {
                ...
                const totalStudents = (await getAllFromIndex(STORES.STUDENTS, 'status', 'active')).length;
        ...
        async function generateStudentDetailsHTML() {
            const students = (await getAllFromIndex(STORES.STUDENTS, 'status', 'active'));
        ...
        async function generateMonthlyCollectionDetailsHTML(month, year) {
            const records = (await getAllFromStore(STORES.FEES)).filter(f => parseInt(f.month) === month && parseInt(f.year) === year);
            const total = records.reduce((sum, rec) => sum + parseFloat(rec.total), 0);
        ...
        async function getTodayAttendancePercentage(date) {
            try {
                ...
                const allStudents = await getAllFromIndex(STORES.STUDENTS, 'status', 'active');
                const activeStudentsInAttendedClasses = allStudents.filter(s => {
                    const classSectionKey = `${s.classId}-${s.sectionId}`;
                    return attendedClassSections.has(classSectionKey);
                });
        ...
        async function getMonthlyFeeCollection(month, year) {
            try {
                ...
                const monthlyRecords = feeRecords.filter(record => {
                    return parseInt(record.month) === month && parseInt(record.year) === year;
                });
                const totalAmount = monthlyRecords.reduce((total, record) => {
                    return total + (parseFloat(record.amount) + parseFloat(record.fine) - parseFloat(record.discount));
                }, 0);
                return totalAmount;
            } catch (error) {
        ...
        async function getTotalPendingFee() {
            try {
                ...
                feeStructures.forEach(fs => {
                    structureMap.set(`${fs.classId}-${fs.category}`, parseFloat(fs.monthlyFee));
                });
                const paymentsMap = new Map();
                allFeeRecords.forEach(rec => {
                    const key = `${rec.studentId}-${rec.month}-${rec.year}`;
                    const totalPaid = (paymentsMap.get(key) || 0) + (parseFloat(rec.amount) - (parseFloat(rec.discount) || 0));
                ...
                for (const student of students) {
                    ...
                    const studentDefaultFee = parseFloat(student.monthlyFee) || 0;
        ...
        function setupFormHandlers() {
            ...
            document.getElementById('fee-structure-form').addEventListener('submit', function(e) {
                e.preventDefault();
                saveFeeStructure();
            });
        ...
        const REPORT_CONFIG = {
            students: [{
                    id: 'student_strength_by_class',
                    name: 'Student Strength by Class',
                    filters: [],
                    generator: generateStudentStrengthByClassReportJS,
                    chartType: 'bar'
                },
                {
                    id: 'student_gender_ratio_by_class',
                    name: 'Student Gender Ratio (Class-wise)',
                    filters: ['class_optional'],
                    generator: generateStudentGenderRatioByClassReportJS,
                    chartType: 'pie_multi'
                },
                {
                    id: 'student_admission_trend',
                    name: 'Student Admission Trend (Monthly)',
                    filters: ['year_optional'],
                    generator: generateStudentAdmissionTrendReportJS,
                    chartType: 'line'
                },
                {
                    id: 'student_list_filtered',
                    name: 'Student List (Filtered)',
                    filters: ['class_optional', 'section_optional', 'gender_optional'],
                    generator: generateFilteredStudentListReportJS,
                    chartType: null
                }
            ],
            attendance: [{
                    ...
                    filters: ['date', 'class_optional', 'section_optional'],
                    generator: generateDailyAttendanceSummaryReportJS,
                    chartType: 'pie'
                },
                {
                    id: 'monthly_attendance_student',
                    name: 'Monthly Attendance per Student',
                    filters: ['month_year', 'class', 'section_optional', 'student_optional'],
                    generator: generateMonthlyAttendanceByStudentReportJS,
                    chartType: 'bar_grouped'
                },
                {
                    id: 'class_attendance_percentage_range',
                    name: 'Class Attendance Percentage (Period)',
                    filters: ['date_range', 'class_optional'],
                    generator: generateClassAttendancePercentageRangeReportJS,
                    chartType: 'bar'
                }
            ],
            fees: [{
                    id: 'fee_collection_daily_summary',
                    name: 'Daily Fee Collection Summary',
                    filters: ['date'],
                    generator: generateFeeCollectionDailySummaryReportJS,
                    chartType: 'multi'
                },
                {
                    id: 'fee_collection_monthly_class',
                    name: 'Monthly Fee Collection (Class-wise)',
                    filters: ['month_year', 'class_optional'],
                    generator: generateFeeCollectionMonthlyByClassReportJS,
                    chartType: 'bar'
                },
                {
                    id: 'fee_defaulters_list',
                    name: 'Fee Defaulters List',
                    filters: ['month_year', 'class_optional', 'section_optional'],
                    generator: generateFeeDefaultersListReportJS,
                    chartType: null
                },
                {
                    id: 'fee_category_wise_collection',
                    name: 'Fee Category-wise Collection (Period)',
                    filters: ['date_range'],
                    generator: generateFeeCategoryWiseCollectionReportJS,
                    chartType: 'pie'
                }
            ],
        };
        ...
        async function initializeReportsViewJS() {
            ...
            const reportTypeGroup = createElementJS('div', {
                class: 'form-group',
                style: {
                    flexBasis: '100%'
                }
            ...
            for (const categoryKey in REPORT_CONFIG) {
                ...
                REPORT_CONFIG[categoryKey].forEach(report => {
                    optgroup.appendChild(createElementJS('option', {
                        value: `${categoryKey}:${report.id}`
                    }, report.name));
                });
        ...
        async function createCollapsibleFilterUI(targetListDivId, filterContainerId, filterTitleTextContent, createFilterRowsFn, populateDropdownsFn, applyFiltersFn, resetFiltersFn, specificDropdownChangeHandlers = []) {
            ...
            const filterToggleIcon = createElementJS('span', {
            ...
            const filterContent = createElementJS('div', {
        ...
        async function createStudentFilterRows(filterContentDiv) {
            const row1 = createElementJS('div', {
                class: 'form-row'
            });
            const classGroup = createElementJS('div', {
                class: 'form-group'
            });
            classGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-student-class'
            }, 'Class'));
            classGroup.appendChild(createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-student-class',
                multiple: true,
                style: {
                    height: '100px'
                }
            ...
            const sectionGroup = createElementJS('div', {
                class: 'form-group'
            });
            sectionGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-student-section'
            }, 'Section'));
            sectionGroup.appendChild(createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-student-section',
                multiple: true,
                style: {
                    height: '100px'
                }
            ...
            const row2 = createElementJS('div', {
                class: 'form-row'
            });
            const genderGroup = createElementJS('div', {
                class: 'form-group'
            });
            genderGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-student-gender'
            }, 'Gender'));
            const genderSelect = createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-student-gender'
            });
            genderSelect.appendChild(createElementJS('option', {
                value: ''
            }, 'All'));
            genderSelect.appendChild(createElementJS('option', {
                value: 'Male'
            ...
            const feeCategoryGroup = createElementJS('div', {
                class: 'form-group'
            });
            feeCategoryGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-student-fee-category'
            }, 'Fee Category'));
            const feeCategorySelect = createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-student-fee-category'
            });
            feeCategorySelect.appendChild(createElementJS('option', {
                value: ''
            }, 'All'));
            ...
            const row3 = createElementJS('div', {
                class: 'form-row'
            });
            const dateFromGroup = createElementJS('div', {
                class: 'form-group'
            });
            dateFromGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-admission-date-from'
            }, 'Admission Date (From)'));
            dateFromGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-admission-date-from'
            }));
            row3.appendChild(dateFromGroup);
            const dateToGroup = createElementJS('div', {
                class: 'form-group'
            });
            dateToGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-admission-date-to'
            }, 'Admission Date (To)'));
            dateToGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-admission-date-to'
            }));
            row3.appendChild(dateToGroup);
            filterContentDiv.appendChild(row3);
            const row4 = createElementJS('div', {
                class: 'form-row'
            });
            row4.appendChild(createElementJS('button', {
                class: 'btn btn-primary',
                id: 'js-apply-student-filters-btn'
            }, 'Apply Filters'));
            row4.appendChild(createElementJS('button', {
                class: 'btn btn-danger',
                id: 'js-reset-student-filters-btn',
                style: {
                    marginLeft: '10px'
                }
            }, 'Reset Filters'));
        ...
        async function populateStudentFilterDropdownsJS() {
            try {
                ...
                classes.forEach(cls => {
                    classFilterDropdown.appendChild(createElementJS('option', {
                        value: cls.id
                    }, cls.name));
                });
            ...
            if (selectedClassIds.length === 0) {
                ...
                for (const section of allSections) {
                    ...
                    sectionFilterDropdown.appendChild(
                        createElementJS('option', {
                            value: section.id
                        }, `${section.name} (${classData?.name || 'N/A'})`)
                    );
                }
                return;
            }
        ...
        async function applyStudentFiltersJS() {
            ...
            try {
                ...
                const students = await fetchStudentsWithFilters(selectedClassIds, selectedSectionIds, selectedGender, selectedFeeCategory, admissionDateFromStr, admissionDateToStr);
        ...
        async function fetchStudentsWithFilters(classIds, sectionIds, gender, feeCategory, admissionDateFrom, admissionDateTo) {
            let url = `indexnewonexact.php?action=get_filtered_students`;
            url += `&classIds=${encodeURIComponent(JSON.stringify(classIds))}`;
            url += `&sectionIds=${encodeURIComponent(JSON.stringify(sectionIds))}`;
            url += `&gender=${encodeURIComponent(gender)}`;
            url += `&feeCategory=${encodeURIComponent(feeCategory)}`;
            url += `&admissionDateFrom=${encodeURIComponent(admissionDateFrom)}`;
            url += `&admissionDateTo=${encodeURIComponent(admissionDateTo)}`;
            const response = await fetch(url);
            const data = await response.json();
            if (data.status === 'success') {
                return data.data;
        ...
        async function createTeacherFilterRows(filterContentDiv) {
            ...
            const subjectGroup = createElementJS('div', {
                class: 'form-group'
            });
            subjectGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-teacher-subject'
            }, 'Subject'));
            subjectGroup.appendChild(createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-teacher-subject',
                multiple: true,
                style: {
                    height: '100px'
                }
            ...
            const qualGroup = createElementJS('div', {
                class: 'form-group'
            });
            qualGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-teacher-qualification'
            }, 'Qualification (Search)'));
            qualGroup.appendChild(createElementJS('input', {
                type: 'text',
                class: 'form-control',
                id: 'js-filter-teacher-qualification',
                placeholder: 'e.g. Alim, Fazil'
            }));
            ...
            dateFromGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-teacher-joining-from'
            }, 'Joining Date (From)'));
            dateFromGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-teacher-joining-from'
            }));
            ...
            dateToGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-teacher-joining-to'
            }, 'Joining Date (To)'));
            dateToGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-teacher-joining-to'
            }));
            ...
            row3.appendChild(createElementJS('button', {
                class: 'btn btn-primary',
                id: 'js-apply-teacher-filters-btn'
            }, 'Apply Filters'));
            row3.appendChild(createElementJS('button', {
                class: 'btn btn-danger',
                id: 'js-reset-teacher-filters-btn',
                style: {
                    marginLeft: '10px'
                }
            }, 'Reset Filters'));
        ...
        async function populateTeacherFilterDropdownsJS() {
            try {
                ...
                subjects.forEach(sub => {
                    subjectFilterDropdown.appendChild(createElementJS('option', {
                        value: sub.id
                    }, sub.name));
                });
            } catch (error) {
        ...
        async function applyTeacherFiltersJS() {
            ...
            try {
                ...
                const teachers = await fetchTeachersWithFilters(selectedSubjectIds, qualificationSearch, joiningDateFromStr, joiningDateToStr);
        ...
        async function fetchTeachersWithFilters(subjectIds, qualificationSearch, joiningDateFrom, joiningDateTo) {
            let url = `indexnewonexact.php?action=get_filtered_teachers`;
            url += `&subjectIds=${encodeURIComponent(JSON.stringify(subjectIds))}`;
            url += `&qualificationSearch=${encodeURIComponent(qualificationSearch)}`;
            url += `&joiningDateFrom=${encodeURIComponent(joiningDateFrom)}`;
            url += `&joiningDateTo=${encodeURIComponent(joiningDateTo)}`;
            const response = await fetch(url);
            const data = await response.json();
            if (data.status === 'success') {
                return data.data;
        ...
        async function createStaffFilterRows(filterContentDiv) {
            ...
            const desigGroup = createElementJS('div', {
                class: 'form-group'
            });
            desigGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-staff-designation'
            }, 'Designation'));
            const desigSelect = createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-staff-designation'
            });
            desigSelect.appendChild(createElementJS('option', {
                value: ''
            }, 'All'));
            ...
            const deptGroup = createElementJS('div', {
                class: 'form-group'
            });
            deptGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-staff-department'
            }, 'Department'));
            const deptSelect = createElementJS('select', {
                class: 'form-control',
                id: 'js-filter-staff-department'
            });
            deptSelect.appendChild(createElementJS('option', {
                value: ''
            }, 'All'));
            ...
            dateFromGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-staff-joining-from'
            }, 'Joining Date (From)'));
            dateFromGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-staff-joining-from'
            }));
            row2.appendChild(dateFromGroup);
            const dateToGroup = createElementJS('div', {
                class: 'form-group'
            ...
            dateToGroup.appendChild(createElementJS('label', {
                class: 'form-label',
                for: 'js-filter-staff-joining-to'
            }, 'Joining Date (To)'));
            dateToGroup.appendChild(createElementJS('input', {
                type: 'date',
                class: 'form-control',
                id: 'js-filter-staff-joining-to'
            }));
            row2.appendChild(dateToGroup);
            filterContentDiv.appendChild(row2);
            const row3 = createElementJS('div', {
            ...
            row3.appendChild(createElementJS('button', {
                class: 'btn btn-primary',
                id: 'js-apply-staff-filters-btn'
            }, 'Apply Filters'));
            row3.appendChild(createElementJS('button', {
                class: 'btn btn-danger',
                id: 'js-reset-staff-filters-btn',
                style: {
                    marginLeft: '10px'
                }
            }, 'Reset Filters'));
        ...
        async function applyStaffFiltersJS() {
            ...
            try {
                ...
                const staffList = await fetchStaffWithFilters(selectedDesignation, selectedDepartment, joiningDateFromStr, joiningDateToStr);
        ...
        async function fetchStaffWithFilters(designation, department, joiningDateFrom, joiningDateTo) {
            let url = `indexnewonexact.php?action=get_filtered_staff`;
        ...
        async function loadStudentsTable(studentsToLoad = null) {
            try {
                const students = studentsToLoad === null ? await getAllFromIndex(STORES.STUDENTS, 'status', 'active') : studentsToLoad;
                ...
                const classMap = new Map(allClasses.map(c => [String(c.id), c.name]));
                const sectionMap = new Map(allSections.map(s => [String(s.id), s.name]));
                const rowsHtml = students.map(student => {
                    const className = classMap.get(String(student.classId)) || 'N/A';
                    const sectionName = sectionMap.get(String(student.sectionId)) || 'N/A';
                    return `
                <tr>
        ...
        async function generateProfileSectionHTML(student) {
            ...
            const displayClassName = classData?.name || 'N/A';
            const displaySectionName = sectionData?.name || 'N/A';
            return `
...
`;
        ...
        async function generateFeeSectionHTML(student) {
            ...
            const paidMap = new Map(paidRecords.map(rec => [`${rec.month}-${rec.year}`, parseFloat(rec.amount) - (parseFloat(rec.discount) || 0)]));
            const dueRecords = [];
            let totalPaid = 0;
            let totalDue = 0;
            const admissionDate = new Date(student.admissionDate);
            ...
            for (let d = new Date(admissionDate); d <= currentDate; d.setMonth(d.getMonth() + 1)) {
                const month = d.getMonth() + 1;
                const monthNameForDisplay = d.toLocaleString('en-US', {
                    month: 'long'
                });
                const year = d.getFullYear();
                const feeStructure = await getFeeStructureForStudent(null, student.classId, student.feeCategory);
                const dueAmount = feeStructure?.monthlyFee || student.monthlyFee || 0;
                const paidForMonth = paidMap.get(`${month}-${year}`) || 0;
                const remainingForMonth = dueAmount - paidForMonth;
                if (remainingForMonth > 0.01) { // Check for small floating point differences
                    dueRecords.push({
                        month: monthNameForDisplay,
                        year,
                        amount: remainingForMonth
                    });
                    totalDue += remainingForMonth;
                }
            }
            ...
            if (paidRecords.length > 0) {
                paidRecords.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(rec => {
                    totalPaid += parseFloat(rec.total);
                    const monthName = new Date(rec.year, rec.month - 1, 1).toLocaleString('en-US', {
                        month: 'long'
                    });
                    html += `<tr>
        ...
        async function showSendNoticePopup(studentId) {
            ...
            try {
                ...
                allPaidRecords.forEach(rec => {
                    ...
                    const paymentInfo = paymentsMap.get(key) || {
                        paid: 0,
                        historicalFee: null
                    };
                    paymentInfo.paid += parseFloat(rec.amount) - (parseFloat(rec.discount) || 0);
                    if (rec.monthlyFeeAtTimeOfPayment) {
                        paymentInfo.historicalFee = parseFloat(rec.monthlyFeeAtTimeOfPayment);
                    }
                ...
                for (let d = new Date(admissionDate.getFullYear(), admissionDate.getMonth(), 1); d <= currentDate; d.setMonth(d.getMonth() + 1)) {
                    ...
                    if (expectedFee === undefined || expectedFee === null) {
                        const feeStructureRate = structureMap.get(`${student.classId}-${student.feeCategory}`) ?? structureMap.get(`${student.classId}-General`);
                        expectedFee = feeStructureRate ?? parseFloat(student.monthlyFee) ?? 0;
                    }
                    if (expectedFee === 0) {
                        continue;
                    }
            ...
            try {
                ...
                for (const classId of classIds) {
                    for (const sectionName of sectionNames) {
                        const id = await addToStore(STORES.SECTIONS, {
                            classId: classId,
                            name: sectionName,
                            inChargeId: null,
                ...
                const subjectsData = [{
                        ...
                        type: 'Quran',
                        periods: 5,
                        details: 'Teaching of Holy Quran'
                    },
                    {
                        name: 'Nazra Quran',
                        code: 'NAZ',
                        type: 'Quran',
                        periods: 5,
                        details: 'Recitation of Holy Quran'
                    },
                    {
                        name: 'Hifz-ul-Quran',
                        code: 'HIF',
                        type: 'Quran',
                        periods: 10,
                        details: 'Memorization of Holy Quran'
                    },
                    {
                        name: 'Tajweed',
                        code: 'TAJ',
                        type: 'Quran',
                        periods: 3,
                        details: 'Correct recitation of Holy Quran'
                    },
                    {
                        ...
                        type: 'Quran',
                        periods: 3,
                        details: 'Exegesis of Holy Quran'
                    },
                    {
                        name: 'Hadith',
                        code: 'HAD',
                        type: 'Hadith',
                        periods: 4,
                        details: 'Teaching of Prophetic Traditions'
                    },
                    {
                        name: 'Fiqh',
                        code: 'FIQ',
                        type: 'Fiqh',
                        periods: 3,
                        details: 'Teaching of Islamic Jurisprudence'
                    },
                    {
                        name: 'Aqeedah',
                        code: 'AQA',
                        type: 'Aqeedah',
                        periods: 2,
                        details: 'Teaching of Islamic Beliefs'
                    },
                    {
                        name: 'Seerat-un-Nabi',
                        code: 'SIR',
                        type: 'Seerah',
                        periods: 2,
                        details: 'Teaching of Prophet Muhammad\'s biography'
                    },
                    {
                        name: 'Ethics',
                        code: 'AKH',
                        type: 'Akhlaq',
                        periods: 2,
                        details: 'Teaching of Islamic Ethics'
                    },
                    {
                        name: 'Arabic',
                        code: 'ARB',
                        type: 'Arabic',
                        periods: 4,
                        details: 'Teaching of Arabic language'
                    },
                    {
                        name: 'Urdu',
                        code: 'URD',
                        type: 'Urdu',
                        periods: 4,
                        details: 'Teaching of Urdu language'
                    },
                    {
                        name: 'English',
                        code: 'ENG',
                        type: 'English',
                        periods: 4,
                        details: 'Teaching of English language'
                    },
                    {
                        name: 'Mathematics',
                        code: 'MAT',
                        type: 'Math',
                        periods: 5,
                        details: 'Teaching of Mathematics'
                    },
                    {
                        name: 'Science',
                        code: 'SCI',
                    ...
                    },
                    {
                        ...
                        type: 'Social Studies',
                        periods: 3,
                        details: 'Teaching of Social Studies'
                    },
                    {
                        ...
                        type: 'Social Studies',
                        periods: 2,
                        details: 'History and Geography of Pakistan'
                    },
                    {
                        name: 'Islamiyat',
                        code: 'ISL',
                        type: 'Islamiyat',
                        periods: 3,
                        details: 'Teaching of Islamic Studies'
                    },
                    {
                        name: 'Computer',
                        code: 'COM',
                        type: 'Computer',
                        periods: 2,
                        details: 'Teaching of Computer Science'
                    },
                    {
                        ...
                        type: 'Health',
                        periods: 2,
                        details: 'Health and Physical Education'
                    },
                ];
                const subjectIds = [];
                ...
                for (let i = 0; i < Math.min(classIds.length, teacherIds.length); i++) {
                    await updateInStore(STORES.CLASSES, classIds[i], {
                        inChargeId: teacherIds[i % teacherIds.length]
                    });
                }
                const staffData = [{
                        employeeId: 'S001',
                ...
                for (const sectionId of sectionIds) {
                    const sectionData = await getFromStore(STORES.SECTIONS, sectionId);
                    const classData = await getFromStore(STORES.CLASSES, sectionData.classId);
                    for (let i = 0; i < 10; i++) {
                ...
                const activitiesData = [{
                        ...
                        date: new Date(new Date().setDate(new Date().getDate() - 1)).toISOString().slice(0, 19).replace('T', ' '),
                        type: 'Admission',
                        user: 'Unknown User',
                        userId: null,
                    ...
                    },
                    {
                        ...
                        date: new Date(new Date().setDate(new Date().getDate() - 2)).toISOString().slice(0, 19).replace('T', ' '),
                        type: 'Fee',
                        user: 'Unknown User',
                        userId: null,
                    ...
                    },
                    {
                        ...
                        date: new Date(new Date().setDate(new Date().getDate() - 3)).toISOString().slice(0, 19).replace('T', ' '),
                        type: 'Library',
                    ...
                    },
                    {
                        ...
                        date: new Date(new Date().setDate(new Date().getDate() - 5)).toISOString().slice(0, 19).replace('T', ' '),
                        type: 'Salary',
                        user: 'Muhammad Arif',
                        userId: 4,
                    ...
                    },
                    {
                        ...
                        date: new Date(new Date().setDate(new Date().getDate() - 7)).toISOString().slice(0, 19).replace('T', ' '),
                        type: 'Exam',
                        user: 'Unknown User',
                        userId: null,
        ...
        async function showSalarySlip(salaryData) {
            ...
            const allPaidRecordsForMonth = (await getAllFromIndex(STORES.SALARY, 'employeeId', salaryData.employeeId)).filter(rec => rec.month === salaryData.month && rec.year === salaryData.year);
            const baseSalary = parseFloat(employee.basicSalary) || 0;
            const allowances = parseFloat(employee.allowances) || 0;
            const grossMonthlySalary = baseSalary + allowances;
            const totalBonusPaid = allPaidRecordsForMonth.reduce((sum, rec) => sum + (parseFloat(rec.bonus) || 0), 0);
            const totalDeductionsPaid = allPaidRecordsForMonth.reduce((sum, rec) => sum + (parseFloat(rec.deductions) || 0), 0);
            const totalAmountPaid = allPaidRecordsForMonth.reduce((sum, rec) => sum + (parseFloat(rec.amountPaid) || 0), 0);
                ...
                switch (viewName) {
                    ...
                    case 'students':
                        ...
                        createCollapsibleFilterUI(
                            ...
                            [{
                                dropdownId: 'js-filter-student-class',
                                handlerFn: populateStudentFilterSectionsJS
                            }]
                        );
                        break;
                    case 'teachers':
        ...
        async function addStudent() {
            ...
            try {
                ...
                if (editId) {
                    studentData.updatedAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
                    await updateInStore(STORES.STUDENTS, editId, studentData);
                    showNotification('Student record updated successfully.', 'success');
                } else {
                    studentData.createdAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
                    await addToStore(STORES.STUDENTS, studentData);
        ...
        async function calculateAndDisplayDueFee(student) {
            ...
            try {
                ...
                allPaidRecords.forEach(rec => {
                    ...
                    existing.paid += parseFloat(rec.amount) - (parseFloat(rec.discount) || 0);
                    if (rec.monthlyFeeAtTimeOfPayment) {
                        existing.historicalFee = parseFloat(rec.monthlyFeeAtTimeOfPayment);
                    }
                    ...
                    if (storeName === STORES.TEACHERS) {
                        ...
                        for (const cls of classes) {
                            if (cls.inChargeId === id) {
                                await updateInStore(STORES.CLASSES, cls.id, {
                                    inChargeId: null
                                });
                            }
                        }
        ...
        async function loadFeeStructureTable() {
            ...
            for (const structure of structures) {
                ...
                const total = (parseFloat(structure.monthlyFee) || 0) + (parseFloat(structure.admissionFee) || 0) + (parseFloat(structure.examFee) || 0) + (parseFloat(structure.computerFee) || 0) + (parseFloat(structure.transportFee) || 0) + (parseFloat(structure.otherFee) || 0);
                const row = document.createElement('tr');
                row.innerHTML = `
            <td>${structure.id}</td>
            <td>${classData ? classData.name : 'N/A'}</td>
        ...
        async function loadSalaryStructureTable() {
            ...
            for (const structure of structures) {
                ...
                const total = (parseFloat(structure.basicSalary) || 0) + (parseFloat(structure.houseRent) || 0) + (parseFloat(structure.transport) || 0) + (parseFloat(structure.medical) || 0) + (parseFloat(structure.otherAllowances) || 0) - (parseFloat(structure.deductions) || 0);
                const row = document.createElement('tr');
                row.innerHTML = `
            <td>${structure.id}</td>
            ...
            <td>${formatCurrency((parseFloat(structure.houseRent) || 0) + (parseFloat(structure.transport) || 0) + (parseFloat(structure.medical) || 0) + (parseFloat(structure.otherAllowances) || 0))}</td>
        ...
        async function loadBackupHistoryTable() {
            ...
            tbody.innerHTML = '';
            const history = (await getAllFromStore(STORES.BACKUP)).filter(b => b.isAutomatic === 0);
        ...
        async function downloadAutomaticBackup(backupId) {
            showNotification('Automatic backup download is not directly supported from history. Please use manual backup for download.', 'info');
        }
            ...
            try {
                ...
                const filteredStudents = allStudents.filter(student => {
                    return student.name.toLowerCase().includes(searchTerm) ||
                        ...
                        (student.classId && String(student.classId).includes(searchTerm)); // Assuming classId might be searched
                });
        ...
        async function viewAttendance() {
            ...
            try {
                const records = (await getAllFromIndex(STORES.ATTENDANCE, 'date', date)).filter(rec => rec.classId === classId && rec.sectionId === sectionId);
                if (records.length === 0) {
        ...
        async function loadStudentForFeeCollection(studentId) {
            ...
            try {
                ...
                const classData = student.classId ? await getFromStore(STORES.CLASSES, student.classId) : null;
                document.getElementById('fee-class').value = classData ? classData.name : 'N/A';
                ...
                const fullMonthlyFee = feeStructure?.monthlyFee || parseFloat(student.monthlyFee) || 0;
                document.getElementById('fee-monthly').value = fullMonthlyFee;
                const allPaidRecords = await getAllFromIndex(STORES.FEES, 'studentId', student.id);
                const paymentsForMonth = allPaidRecords
                    .filter(rec => parseInt(rec.month) === selectedMonth && parseInt(rec.year) === selectedYear)
                    .reduce((sum, rec) => sum + (parseFloat(rec.amount) - (parseFloat(rec.discount) || 0)), 0);
        ...
        async function loadFeeHistory(studentId) {
            ...
            allFees.forEach(fee => {
                const row = document.createElement('tr');
                const monthName = new Date(fee.year, fee.month - 1, 1).toLocaleString('en-US', {
                    month: 'long'
                });
                row.innerHTML = `
        ...
        async function generateFeeReport() {
            ...
            try {
                ...
                let filteredFees = [];
                let studentsInScope = allStudents.filter(s => s.status === 'active');
                ...
                switch (reportType) {
                    ...
                    case 'monthly':
                        ...
                        filteredFees = allFees.filter(fee => parseInt(fee.month) === month && parseInt(fee.year) === year);
                        break;
                    case 'yearly':
                        if (!year) {
                        ...
                        filteredFees = allFees.filter(fee => parseInt(fee.year) === year);
                        break;
                    case 'class':
                        if (!classId || !month || !year) {
                            showNotification('Please select class, month, and year.', 'error');
                            return;
                        }
                        ...
                        filteredFees = allFees.filter(fee =>
                            ...
                            parseInt(fee.month) === month &&
                            parseInt(fee.year) === year
                        );
                        break;
                    default:
                ...
                for (const fee of filteredFees) {
                    ...
                    totalCollected += parseFloat(fee.total);
        ...
        async function generateDefaultersReport() {
            ...
            container.style.display = 'none';
            try {
                let students = (await getAllFromIndex(STORES.STUDENTS, 'status', 'active'));
                ...
                feeRecordsForMonth.forEach(rec => {
                    ...
                    paymentsMap.set(rec.studentId, currentPaid + (parseFloat(rec.amount) - (parseFloat(rec.discount) || 0)));
                });
                if (classId !== 'all') {
                ...
                for (const student of students) {
                    ...
                    const feeStructureRate = structureMap.get(`${student.classId}-${student.feeCategory}`) ?? structureMap.get(`${student.classId}-General`);
                    const expectedFee = feeStructureRate ?? parseFloat(student.monthlyFee) ?? 0;
                    if (expectedFee === 0) continue;
        ...
        async function searchEmployeeForSalary() {
            ...
            try {
                ...
                const fullSalary = (parseFloat(employee.basicSalary) || 0) + (parseFloat(employee.allowances) || 0);
                ...
                const paymentsForMonth = allPaidRecords
                    ...
                    .reduce((sum, rec) => sum + parseFloat(rec.total), 0);
        ...
        async function loadSalaryHistory(employeeId, employeeType) {
            const allSalaries = (await getAllFromIndex(STORES.SALARY, 'employeeId', employeeId)).filter(s => s.employeeType === employeeType);
        ...
        async function generateSalaryReport() {
            ...
            try {
                ...
                for (const salary of filteredSalaries) {
                    ...
                    totalPaid += (parseFloat(salary.total) || 0);
                    paidEmployees.add(salary.employeeId);
                    const row = document.createElement('tr');
                    const employeeName = employee ? employee.name : 'N/A';
                    const employeeDesignation = employee ? (employee.designation || employee.specialization) : 'N/A';
                    row.innerHTML = `
                        <td>${salary.voucherNumber || '-'}</td>
                        <td>${employeeName}</td>
                ...
                let totalEmployeesInScope = 0;
                if (employeeTypeFilter === 'all') totalEmployeesInScope = allTeachersCount + allStaffCount;
        ...
        async function loadEmployeesForSalaryReport() {
            try {
                ...
                if (employeeType === 'all') {
                    ...
                    employees = (await getAllFromStore(STORES.TEACHERS)).map(t => ({
                        ...t,
                        type: 'teacher'
                    }));
                } else {
                    employees = (await getAllFromStore(STORES.STAFF)).map(s => ({
                        ...s,
                        type: 'staff'
                    }));
                }
            ...
            try {
                ...
                if (editId) {
                    ...
                    if (existingStaff) {
                        ...
                        staffData.updatedAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
                    ...
                    staffData.createdAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
        ...
        async function addTeacher() {
            ...
            try {
                ...
                if (editId) {
                    teacherData.updatedAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
                    await updateInStore(STORES.TEACHERS, editId, teacherData);
                    showNotification('Teacher record updated successfully.', 'success');
                } else {
                    teacherData.createdAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
        ...
        async function saveClassSubject() {
            try {
                ...
                const existingClassSubjects = await getAllFromIndex(STORES.CLASS_SUBJECTS, 'classId', parseInt(classId));
        ...
        async function saveAttendance() {
            try {
                ...
                for (const row of rows) {
                    ...
                    const isLate = row.querySelector('input[name="late_' + studentId + '"]').checked ? 1 : 0;
                    const isLeave = row.querySelector('input[name="leave_' + studentId + '"]').checked ? 1 : 0;
                ...
                const existingAttendance = (await getAllFromIndex(STORES.ATTENDANCE, 'date', date)).filter(rec => rec.classId === classId && rec.sectionId === sectionId);
            ...
            try {
                ...
                for (const row of rows) {
                    ...
                    marksData.push({
                        ...
                        grade: grade,
                        isPassed: percentage >= 33 ? 1 : 0,
                        notes: notes,
                        createdAt: new Date().toISOString().slice(0, 19).replace('T', ' ')
                    });
                }
                for (const markRecord of marksData) {
                    const existingMarks = (await getAllFromIndex(STORES.MARKS, 'examId', markRecord.examId))
                        .filter(m => m.studentId === markRecord.studentId && m.subjectId === markRecord.subjectId);
                    if (existingMarks && existingMarks.length > 0) {
                        await updateInStore(STORES.MARKS, existingMarks[0].id, markRecord);
                    } else {
        ...
        async function collectFee() {
            try {
                ...
                const student = await getFromStore(STORES.STUDENTS, studentId);
                const feeStructure = await getFeeStructureForStudent(student.id, student.classId, student.feeCategory);
                const monthlyFeeNow = parseFloat(feeStructure?.monthlyFee || student.monthlyFee || 0);
        ...
        async function saveFeeStructure() {
            try {
                ...
                const existingStructure = (await getAllFromIndex(STORES.FEE_STRUCTURE, 'classId', classId)).filter(fs => fs.category === category);
        ...
        async function paySalary() {
            try {
                ...
                const grossSalaryNow = (parseFloat(employee.basicSalary) || 0) + (parseFloat(employee.allowances) || 0);
                ...
                const salaryData = {
                    ...
                    grossSalaryAtTimeOfPayment: grossSalaryNow,
        ...
        async function saveSalaryStructure() {
            try {
                ...
                const existingStructure = (await getAllFromIndex(STORES.SALARY_STRUCTURE, 'employeeId', employeeId)).filter(s => s.employeeType === employeeType);
                if (existingStructure && existingStructure.length > 0) {
        ...
        async function returnBook() {
            try {
                ...
                await addToStore(STORES.ACTIVITIES, {
                    ...
                    user: currentUser ? currentUser.name : 'Unknown User',
                    userId: currentUser ? currentUser.id : null,
                    details: `${book.name} returned by ${issue.memberName || 'Student/Teacher'}`,
                    createdAt: new Date().toISOString().slice(0, 19).replace('T', ' ')
                });
        ...
        async function saveGeneralSettings() {
            try {
                ...
                await addToStore(STORES.SETTINGS, updatedSettings, 'general');
        ...
        async function createBackup() {
            try {
                ...
                formData.append('backupFormat', backupFormat);
                formData.append('isAutomatic', 'false');
                ...
                if (result.status === 'success') {
                    ...
                    // Trigger download manually for user
                    const downloadLink = document.createElement('a');
                    downloadLink.href = `./backups/${result.filename}`;
                    downloadLink.download = result.filename;
                    document.body.appendChild(downloadLink);
                    downloadLink.click();
                    document.body.removeChild(downloadLink);
                    loadBackupHistoryTable(); // Refresh history
                } else {
        ...
        async function loadStudentsForAttendance() {
            ...
            try {
                ...
                if (classStudents.length === 0) {
                    ...
                    const existingAttendanceRecords = (await getAllFromIndex(STORES.ATTENDANCE, 'date', date)).filter(rec => rec.classId === classId && rec.sectionId === sectionId);
                    ...
                    classStudents.forEach(student => {
                        ...
                        const isLate = existingRecord ? existingRecord.isLate === 1 : false; // Check for boolean 1
                        const onLeave = existingRecord ? existingRecord.isLeave === 1 : false; // Check for boolean 1
        ...
        async function saveAutoBackupSettings() {
            try {
                ...
                const updatedSettings = {
                    autoBackupEnabled: isEnabled ? 1 : 0,
    ...
    `;
        ...
        async function renderFiltersForReport(filters, container) {
            for (const filterType of filters) {
                const filterGroup = createElementJS('div', {
                    class: 'form-group'
                });
                let requiresRepopulate = false;
                switch (filterType) {
                    case 'date':
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-date'
                        }, 'Date'));
                        filterGroup.appendChild(createElementJS('input', {
                            type: 'date',
                            class: 'form-control',
                            id: 'js-filter-date',
                    ...
                    case 'date_range':
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-date-from'
                        }, 'Date (From)'));
                        filterGroup.appendChild(createElementJS('input', {
                            type: 'date',
                            class: 'form-control',
                            id: 'js-filter-date-from'
                        }));
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-date-to',
                            style: {
                                marginTop: '5px'
                            }
                        }, 'Date (To)'));
                    ...
                    case 'month_year':
                        ...
                        const monthSelect = createElementJS('select', {
                            class: 'form-control',
                            id: 'js-filter-month'
                        });
                        const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
                        months.forEach((m, i) => monthSelect.appendChild(createElementJS('option', {
                            value: i + 1
                        }, m)));
                    ...
                    case 'year_optional':
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-year-optional'
                        }, 'Year (Optional)'));
                        const yearOptionalInput = createElementJS('input', {
                            type: 'number',
                            class: 'form-control',
                            id: 'js-filter-year-optional',
                    ...
                    case 'class_optional':
                        ...
                        const classSelect = createElementJS('select', {
                            class: 'form-control',
                            id: 'js-filter-class'
                        });
                        if (filterType === 'class_optional') {
                            classSelect.appendChild(createElementJS('option', {
                                value: ''
                            }, 'All Classes'));
                        }
                        const classes = await getAllFromStore(STORES.CLASSES);
                        classes.forEach(c => classSelect.appendChild(createElementJS('option', {
                            value: c.id
                        }, c.name)));
                    ...
                    case 'section_optional':
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-section'
                        }, 'Section'));
                        const sectionSelect = createElementJS('select', {
                            class: 'form-control',
                            id: 'js-filter-section'
                        });
                    ...
                    case 'student_optional':
                        filterGroup.appendChild(createElementJS('label', {
                            class: 'form-label',
                            for: 'js-filter-student'
                        }, 'Student'));
                        const studentSelect = createElementJS('select', {
                            class: 'form-control',
                            id: 'js-filter-student'
                        });
        ...
        async function populateReportFilterSectionsJS(isOptional = false) {
            ...
            sectionSelect.innerHTML = '';
            if (isOptional) {
                sectionSelect.appendChild(createElementJS('option', {
                    value: ''
                }, 'All Sections'));
            }
            ...
            if (classId && !isNaN(classId)) {
                const sections = await getAllFromIndex(STORES.SECTIONS, 'classId', classId);
                sections.forEach(s => sectionSelect.appendChild(createElementJS('option', {
                    value: s.id
                }, s.name)));
        ...
        async function populateReportFilterStudentsJS() {
            ...
            studentSelect.appendChild(createElementJS('option', {
                value: ''
            }, 'All Students'));
            ...
            students.forEach(s => studentSelect.appendChild(createElementJS('option', {
                value: s.id
            }, `${s.name || 'Unnamed'} (${s.rollNumber || 'N/A'})`)));
        }
        async function generateSelectedReportJS() {
            ...
            try {
                ...
                const {
                    html,
                    chartData
                } = await reportConfig.generator(filterValues);
        ...
        async function generatePendingFeeDetailsHTML() {
            ...
            for (const student of students) {
                ...
                if (!student.admissionDate) continue;
                const feeStructureRate = structureMap.get(`${student.classId}-${student.feeCategory}`) ?? structureMap.get(`${student.classId}-General`);
                const currentMonthlyFee = feeStructureRate ?? parseFloat(student.monthlyFee) ?? 0;
                ...
                for (let d = new Date(admissionDate.getFullYear(), admissionDate.getMonth(), 1); d <= currentDate; d.setMonth(d.getMonth() + 1)) {
                    ...
                    const paymentKey = `${student.id}-${month}-${year}`;
                    const paymentInfo = paymentsMap.get(paymentKey);
                    let expectedFee = paymentInfo?.historicalFee ?? feeStructureRate ?? parseFloat(student.monthlyFee) ?? 0;
                    if (expectedFee === 0) continue;
                    const paidForMonth = paymentInfo?.paid || 0;
        ...
        async function generateMonthlyAttendanceByStudentReportJS(filters) {
            ...
            if (!month || !year || !classId) {
                return {
                    html: "<p>Please select Month, Year, and Class. Section is optional.</p>",
                    chartData: null
                };
            }
            ...
            if (studentsToReport.length === 0) return {
                html: html + "<p>No students found for this criteria.</p>",
                chartData: null
            };
            const allAttendance = await getAllFromStore(STORES.ATTENDANCE);
            ...
            for (const student of studentsToReport) {
                ...
                const present = studentMonthlyAttendance.filter(r => r.status === 'present' && r.isLeave === 0).length;
                const absent = studentMonthlyAttendance.filter(r => r.status === 'absent' && r.isLeave === 0).length;
                const onLeave = studentMonthlyAttendance.filter(r => r.isLeave === 1).length;
                const late = studentMonthlyAttendance.filter(r => r.isLate === 1).length;
        ...
        async function generateClassAttendancePercentageRangeReportJS(filters) {
            ...
            if (!dateFrom || !dateTo) {
                return {
                    html: "<p>Please select a date range.</p>",
                    chartData: null
                };
            }
            let classesToReport = [];
            ...
            if (classesToReport.length === 0) return {
                html: "<p>No classes selected or available.</p>",
                chartData: null
            };
            const allAttendance = await getAllFromStore(STORES.ATTENDANCE);
            const allStudents = await getAllFromStore(STORES.STUDENTS);
            ...
            for (const cls of classesToReport) {
                ...
                for (const student of studentsInClass) {
                    ...
                    const nonLeaveRecords = studentAttendanceInRange.filter(r => r.isLeave === 0);
        ...
        async function generateFeeCollectionDailySummaryReportJS(filters) {
            ...
            feeRecords.forEach(rec => {
                totalCollected += parseFloat(rec.total) || 0;
                byMethod[rec.paymentMethod] = (byMethod[rec.paymentMethod] || 0) + (parseFloat(rec.total) || 0);
                const student = studentMap.get(rec.studentId);
                if (student && student.classId) {
                    const className = classMap.get(student.classId)?.name || 'Other';
                    byClass[className] = (byClass[className] || 0) + (parseFloat(rec.total) || 0);
                }
            });
            ...
            charts.push({
                ...
                datasets: [{
                    ...
                    backgroundColor: Object.keys(byMethod).map(() => `rgba(${Math.floor(Math.random() * 200) + 55}, ${Math.floor(Math.random() * 200) + 55}, ${Math.floor(Math.random() * 200) + 55}, 0.7)`)
                }]
            });
        ...
        async function generateFeeCollectionMonthlyByClassReportJS(filters) {
            ...
            if (!month || !year) return {
                html: "<p>Please select Month and Year.</p>",
                chartData: null
            };
            let classesToReport = [];
            if (classId && !isNaN(classId)) {
            ...
            for (const cls of classesToReport) {
                ...
                for (const student of studentsInClass) {
                    ...
                    classTotalExpected += parseFloat(feeStructure?.monthlyFee || student.monthlyFee || 0);
                    const studentFeeRecord = feeRecords.find(fr => fr.studentId === student.id);
                    if (studentFeeRecord) {
                        classTotalCollected += parseFloat(studentFeeRecord.total) || 0;
                        classPaidCount++;
                    }
                }
        ...
        async function getFeeStructureForStudent(studentId, classId, feeCategory) {
            try {
                const response = await fetch(`indexnewonexact.php?action=fetch_fee_structure_for_student&classId=${classId}&feeCategory=${feeCategory}`);
                const data = await response.json();
                if (data.status === 'success' && data.structure) {
                    ...
                    Object.keys(data.structure).forEach(key => {
                        if (typeof data.structure[key] === 'string' && !isNaN(parseFloat(data.structure[key]))) {
                            data.structure[key] = parseFloat(data.structure[key]);
                        }
                    });
                ...
                console.error("Error fetching fee structure for student:", error);
                return {
                    monthlyFee: 0,
                    admissionFee: 0,
                    examFee: 0,
        ...
        async function generateFeeDefaultersListReportJS(filters) {
            ...
            const defaulters = studentsToList.filter(s => s.status === 'active' && !paidStudentIds.has(s.id));
            ...
            for (const student of defaulters) {
                ...
                const sectionData = student.sectionId ? await getFromStore(STORES.SECTIONS, student.sectionId) : null;
                const feeStructure = await getFeeStructureForStudent(student.id, student.classId, student.feeCategory);
                const expectedFee = feeStructure?.monthlyFee || parseFloat(student.monthlyFee) || 0;
        ...
        async function generateFeeCategoryWiseCollectionReportJS(filters) {
            ...
            feeRecords.forEach(rec => {
                ...
                collectionByCategory[category] = (collectionByCategory[category] || 0) + (parseFloat(rec.total) || 0);
                totalCollected += (parseFloat(rec.total) || 0);
            });
        ...
        function setupCertificateInteraction(studentId) {
            ...
            const prevButton = document.getElementById('cert-prev-frame');
            if (prevButton) {
                prevButton.removeEventListener('click', () => {}); // Remove old listeners
            ...
            const nextButton = document.getElementById('cert-next-frame');
            if (nextButton) {
                nextButton.removeEventListener('click', () => {}); // Remove old listeners
            ...
            editableElements.forEach(el => {
                ...
                el.removeEventListener('blur', () => {}); // Remove old listeners
    ...
    <div class="certificate-wrapper" id="cert-wrapper-${student.id}">
        ...
        <div class="certificate-container" id="cert-container-${student.id}">
            ...
            setupCertificateInteraction(student.id);
        ...
        async function handleImageUpload(event, memberId, memberType) {
            ...
            reader.onload = async (e) => {
                ...
                await updateInStore(storeName, memberId, {
                    photo: b64
                });
                hideLoading();
            ...
            const translations = {
                /*                 'search': {
                                ...
                                'admission_date': {
                                    ...
                                    ur: 'داخلے کی تاریخ',
                                    isGeneric: true,
                                    matchingText: '^Admission Date$'
                                },
                                'joining_date': {
                                    selector: 'label[for$="-joining-date"]',
                                    en: 'Joining Date',
                                    ur: 'شمولیت کی تاریخ',
                                    isGeneric: true,
                                    matchingText: 'Joining Date'
                                },
                                'notes': {
                                    selector: 'label.form-label',
                                    en: 'Note',
                                ...
                                'details': {
                                    selector: '.table th, label.form-label',
                                    en: 'Details',
                                    ur: 'تفصیلات',
                                    isGeneric: true,
                                    matchingText: 'Details'
                                },
                                'additional_info': {
                                    selector: 'label.form-label',
                                    en: 'Additional Info',
                                ...
                                'photo': {
                                    ...
                                    ur: 'تصویر',
                                    isGeneric: true,
                                    matchingText: 'Student Photo|Teacher Photo|Staff Photo|User Photo'
                                },
                                'header_title': {
                                    selector: '.header h1',
                                    en: 'School & Madrasa Management App',
                                    ur: 'اسکول و مدرسہ مینجمنٹ ایپ'
                                },
                                'sidebar_dashboard': {
                                    selector: 'li[data-view="dashboard"]',
                                    en: 'Dashboard',
                                    ur: 'ڈیش بورڈ'
                                },
                                'sidebar_students': {
                                    selector: 'li[data-view="students"]',
                                    en: 'Students',
                                    ur: 'طلباء'
                                },
                                'sidebar_teachers': {
                                    selector: 'li[data-view="teachers"]',
                                    en: 'Teachers',
                                    ur: 'اساتذہ'
                                },
                                'sidebar_staff': {
                                    selector: 'li[data-view="staff"]',
                                    en: 'Staff',
                                    ur: 'عملہ'
                                },
                                'sidebar_classes': {
                                    selector: 'li[data-view="classes"]',
                                    en: 'Classes',
                                    ur: 'کلاسیں'
                                },
                                'sidebar_subjects': {
                                    selector: 'li[data-view="subjects"]',
                                    en: 'Subjects',
                                    ur: 'مضامین'
                                },
                                'sidebar_attendance': {
                                    selector: 'li[data-view="attendance"]',
                                    en: 'Attendance',
                                    ur: 'حاضری'
                                },
                                'sidebar_exams': {
                                    selector: 'li[data-view="exams"]',
                                    en: 'Exams',
                                    ur: 'امتحانات'
                                },
                                'sidebar_fees': {
                                    selector: 'li[data-view="fees"]',
                                    en: 'Fees',
                                    ur: 'فیس'
                                },
                                'sidebar_salaries': {
                                    selector: 'li[data-view="salaries"]',
                                    en: 'Salaries',
                                    ur: 'تنخواہیں'
                                },
                                'sidebar_library': {
                                    selector: 'li[data-view="library"]',
                                    en: 'Library',
                                    ur: 'لائبریری'
                                },
                                'sidebar_users': {
                                    selector: 'li[data-view="users"]',
                                    en: 'Users',
                                    ur: 'صارفین'
                                },
                                'sidebar_reports': {
                                    selector: 'li[data-view="reports"]',
                                    en: 'Reports',
                                    ur: 'رپورٹس'
                                },
                                'sidebar_settings': {
                                    selector: 'li[data-view="settings"]',
                                    en: 'Settings',
                                    ur: 'ترتیبات'
                                },
                                'sidebar_backup': {
                                    selector: 'li[data-view="backup"]',
                                    en: 'Backup',
                                    ur: 'بیک اپ'
                                },
                                'sidebar_idcards': {
                                    selector: 'li[data-view="id-cards"]',
                                    en: 'ID Cards & Certificates',
                                    ur: 'شناختی کارڈ اور سرٹیفکیٹ'
                                },
                                'dashboard_title': {
                                    selector: '#dashboard-view > h2',
                                    en: 'Dashboard',
                                    ur: 'ڈیش بورڈ'
                                },
                                'dash_total_students': {
                                    selector: '.card:has(#total-students) .card-title',
                                    en: 'Total Students',
                                    ur: 'کل طلباء'
                                },
                                'dash_total_teachers': {
                                    selector: '.card:has(#total-teachers) .card-title',
                                    en: 'Total Teachers',
                                    ur: 'کل اساتذہ'
                                },
                                'dash_total_classes': {
                                    selector: '.card:has(#total-classes) .card-title',
                                    en: 'Total Classes',
                                    ur: 'کل کلاسیں'
                                },
                                'dash_today_attendance': {
                                    selector: '.card:has(#today-attendance) .card-title',
                                    en: "Today's Attendance",
                                    ur: 'آج کی حاضری'
                                },
                                'dash_monthly_fee': {
                                    selector: '.card:has(#monthly-fee) .card-title',
                                    en: 'Monthly Fee Collection',
                                    ur: 'ماہانہ فیس وصولی'
                                },
                                'dash_pending_fee': {
                                    selector: '.card:has(#pending-fee) .card-title',
                                    en: 'Outstanding Fees',
                                    ur: 'بقایا فیس'
                                },
                                'dash_events': {
                                    selector: '#dashboard-view .form-container:nth-of-type(1) .form-title',
                                    en: 'Upcoming Events',
                                    ur: 'آنے والے واقعات'
                                },
                                'dash_activities': {
                                    selector: '#dashboard-view .form-container:nth-of-type(2) .form-title',
                                    en: 'Recent Activities',
                                    ur: 'حالیہ سرگرمیاں'
                                },
                                'students_title': {
                                    selector: '#students-view > h2',
                                    en: 'Students',
                                    ur: 'طلباء'
                                },
                                'students_tab_list': {
                                    selector: '#students-view .tab[data-tab="students-list"]',
                                    en: 'Student List',
                                    ur: 'طلباء کی فہرست'
                                },
                                'students_tab_add': {
                                    selector: '#students-view .tab[data-tab="add-student"]',
                                    en: 'Add New Student',
                                    ur: 'نیا طالب علم شامل کریں'
                                },
                                'students_search_placeholder': {
                                    selector: '#student-search',
                                    prop: 'placeholder',
                                    en: 'Search by student name, roll number or class',
                                    ur: 'نام، رول نمبر یا کلاس سے تلاش کریں'
                                },
                                'students_form_title': {
                                    selector: '#add-student .form-title',
                                    en: 'Add New Student',
                                    ur: 'نیا طالب علم شامل کریں'
                                },
                                'students_form_add_btn': {
                                    selector: '#student-form .btn-primary',
                                    en: 'Add/Update Student',
                                    ur: 'طالب علم شامل کریں'
                                },
                                'teachers_title': {
                                    selector: '#teachers-view > h2',
                                    en: 'Teachers',
                                    ur: 'اساتذہ'
                                },
                                'teachers_tab_list': {
                                    selector: '#teachers-view .tab[data-tab="teachers-list"]',
                                    en: 'Teacher List',
                                    ur: 'اساتذہ کی فہرست'
                                },
                                'teachers_tab_add': {
                                    selector: '#teachers-view .tab[data-tab="add-teacher"]',
                                    en: 'Add New Teacher',
                                ...
                                'teachers_search_placeholder': {
                                    ...
                                    prop: 'placeholder',
                                    en: 'Search by teacher name, subject or class',
                                    ur: 'نام، مضمون یا کلاس سے تلاش کریں'
                                },
                                'teachers_form_title': {
                                    selector: '#add-teacher .form-title',
                                    en: 'Add New Teacher',
                                    ur: 'نیا استاد شامل کریں'
                                },
                                'teachers_form_qualification': {
                                    selector: 'label[for="teacher-qualification"]',
                                    en: 'Qualification',
                                    ur: 'قابلیت'
                                },
                                'teachers_form_specialization': {
                                    selector: 'label[for="teacher-specialization"]',
                                    en: 'Specialization',
                                    ur: 'مہارت'
                                },
                                'teachers_form_subjects': {
                                    selector: 'label[for="teacher-subject"]',
                                    en: 'Teacher Subjects',
                                    ur: 'مضامین'
                                },
                                'teachers_form_classes': {
                                    selector: 'label[for="teacher-class"]',
                                    en: 'Classes',
                                    ur: 'کلاسیں'
                                },
                                'teachers_form_cnic': {
                                    selector: 'label[for="teacher-cnic"]',
                                    en: 'CNIC No.',
                                    ur: 'شناختی کارڈ نمبر'
                                },
                                'teachers_form_joining': {
                                    selector: 'label[for="teacher-joining-date"]',
                                    en: 'Joining Date',
                                    ur: 'شمولیت کی تاریخ'
                                },
                                'teachers_form_salary': {
                                    selector: 'label[for="teacher-basic-salary"]',
                                    en: 'Basic Salary',
                                    ur: 'بنیادی تنخواہ'
                                },
                                'teachers_form_add_btn': {
                                    selector: '#teacher-form .btn-primary',
                                    en: 'Add/Update Teacher',
                                    ur: 'استاد شامل کریں'
                                },
                                'staff_title': {
                                    selector: '#staff-view > h2',
                                    en: 'Staff',
                                    ur: 'عملہ'
                                },
                                'staff_tab_list': {
                                    selector: '#staff-view .tab[data-tab="staff-list"]',
                                    en: 'Staff List',
                                    ur: 'عملے کی فہرست'
                                },
                                'staff_tab_add': {
                                    selector: '#staff-view .tab[data-tab="add-staff"]',
                                    en: 'Add New Staff',
                                ...
                                'staff_search_placeholder': {
                                    ...
                                    prop: 'placeholder',
                                    en: 'Search by staff name, designation or department',
                                    ur: 'نام، عہدہ یا محکمہ سے تلاش کریں'
                                },
                                'staff_form_title': {
                                    selector: '#add-staff .form-title',
                                    en: 'Add New Staff',
                                    ur: 'نیا عملہ شامل کریں'
                                },
                                'staff_form_designation': {
                                    selector: 'label[for="staff-designation"]',
                                    en: 'Designation',
                                    ur: 'عہدہ'
                                },
                                'staff_form_department': {
                                    selector: 'label[for="staff-department"]',
                                    en: 'Department',
                                    ur: 'محکمہ'
                                },
                                'staff_form_responsibilities': {
                                    selector: 'label[for="staff-responsibilities"]',
                                    en: 'ذمہ داریاں',
                                    ur: 'ذمہ داریاں'
                                },
                                'staff_form_add_btn': {
                                    selector: '#staff-form .btn-primary',
                                    en: 'Add Staff',
                                    ur: 'عملہ شامل کریں'
                                },
                                'classes_title': {
                                    selector: '#classes-view > h2',
                                    en: 'Classes',
                                    ur: 'کلاسیں'
                                },
                                'classes_tab_list': {
                                    selector: '#classes-view .tab[data-tab="classes-list"]',
                                    en: 'Class List',
                                    ur: 'کلاسوں کی فہرست'
                                },
                                'classes_tab_add': {
                                    selector: '#classes-view .tab[data-tab="add-class"]',
                                    en: 'Add New Class',
                                    ur: 'نئی کلاس شامل کریں'
                                },
                                'classes_tab_sections': {
                                    selector: '#classes-view .tab[data-tab="sections"]',
                                    en: 'Sections',
                                    ur: 'سیکشنز'
                                },
                                'classes_form_title': {
                                    selector: '#add-class .form-title',
                                    en: 'Add New Class',
                                    ur: 'نئی کلاس شامل کریں'
                                },
                                'classes_form_incharge': {
                                    selector: 'label[for="class-incharge"]',
                                    en: 'Class Incharge',
                                    ur: 'کلاس انچارج'
                                },
                                'classes_form_level': {
                                    selector: 'label[for="class-level"]',
                                    en: 'Class Level',
                                    ur: 'کلاس کا درجہ'
                                },
                                'classes_form_room': {
                                    selector: 'label[for="class-room"]',
                                    en: 'Room No.',
                                    ur: 'کمرہ نمبر'
                                },
                                'classes_form_fee_cat': {
                                    selector: 'label[for="class-fee-category"]',
                                    en: 'Fee Category',
                                    ur: 'فیس کیٹیگری'
                                },
                                'classes_form_add_btn': {
                                    selector: '#class-form .btn-primary',
                                    en: 'Add Class',
                                    ur: 'کلاس شامل کریں'
                                },
                                'classes_section_title': {
                                    selector: '#sections .form-title',
                                    en: 'Section Management',
                                    ur: 'سیکشن مینجمنٹ'
                                },
                                'classes_section_incharge': {
                                    selector: 'label[for="section-incharge"]',
                                    en: 'Section Incharge',
                                    ur: 'سیکشن انچارج'
                                },
                                'classes_section_save_btn': {
                                    selector: '#save-section',
                                    en: 'Save Section',
                                    ur: 'سیکشن محفوظ کریں'
                                },
                                'subjects_title': {
                                    selector: '#subjects-view > h2',
                                    en: 'Subjects',
                                    ur: 'مضامین'
                                },
                                'subjects_tab_list': {
                                    selector: '#subjects-view .tab[data-tab="subjects-list"]',
                                    en: 'Subject List',
                                    ur: 'مضامین کی فہرست'
                                },
                                'subjects_tab_add': {
                                    selector: '#subjects-view .tab[data-tab="add-subject"]',
                                    en: 'Add New Subject',
                                    ur: 'نیا مضمون شامل کریں'
                                },
                                'subjects_tab_class': {
                                    selector: '#subjects-view .tab[data-tab="class-subjects"]',
                                    en: 'Class Subjects',
                                    ur: 'کلاس کے مضامین'
                                },
                                'subjects_form_title': {
                                    selector: '#add-subject .form-title',
                                    en: 'Add New Subject',
                                    ur: 'نیا مضمون شامل کریں'
                                },
                                'subjects_form_code': {
                                    selector: 'label[for="subject-code"]',
                                    en: 'Subject Code',
                                    ur: 'مضمون کا کوڈ'
                                },
                                'subjects_form_type': {
                                    selector: 'label[for="subject-type"]',
                                    en: 'Subject Type',
                                    ur: 'مضمون کی قسم'
                                },
                                'subjects_form_periods': {
                                    selector: 'label[for="subject-periods"]',
                                    en: 'Weekly Periods',
                                    ur: 'ہفتہ وار پیریڈز'
                                },
                                'subjects_form_add_btn': {
                                    selector: '#subject-form .btn-primary',
                                    en: 'Add Subject',
                                    ur: 'مضمون شامل کریں'
                                },
                                'subjects_class_title': {
                                    selector: '#class-subjects .form-title',
                                    en: 'Class Subjects',
                                    ur: 'کلاس کے مضامین'
                                },
                                'subjects_class_teacher': {
                                    selector: 'label[for="class-subject-teacher"]',
                                    en: 'Teacher',
                                    ur: 'استاد'
                                },
                                'subjects_class_save_btn': {
                                    selector: '#save-class-subject',
                                    en: 'Save Subject',
                                    ur: 'مضمون محفوظ کریں'
                                },
                                'attendance_title': {
                                    selector: '#attendance-view > h2',
                                    en: 'Attendance',
                                    ur: 'حاضری'
                                },
                                'attendance_tab_mark': {
                                    selector: '#attendance-view .tab[data-tab="mark-attendance"]',
                                    en: 'Mark Attendance',
                                    ur: 'حاضری لگائیں'
                                },
                                'attendance_tab_view': {
                                    selector: '#attendance-view .tab[data-tab="view-attendance"]',
                                    en: 'View Attendance',
                                    ur: 'حاضری دیکھیں'
                                },
                                'attendance_tab_report': {
                                    selector: '#attendance-view .tab[data-tab="attendance-report"]',
                                ...
                                'attendance_save_btn': {
                                    selector: '#attendance-form .btn-primary',
                                    en: 'Save Attendance',
                                    ur: 'حاضری محفوظ کریں'
                                },
                                'attendance_view_btn': {
                                    selector: '#view-attendance-btn',
                                    en: 'View Attendance',
                                    ur: 'حاضری دیکھیں'
                                },
                                'attendance_generate_report_btn': {
                                    selector: '#generate-attendance-report',
                                    en: 'Generate Report',
                                    ur: 'رپورٹ بنائیں'
                                },
                                'fees_title': {
                                    selector: '#fees-view > h2',
                                    en: 'Fees',
                                    ur: 'فیس'
                                },
                                'fees_tab_collect': {
                                    selector: '#fees-view .tab[data-tab="fee-collection"]',
                                    en: 'Collect Fee',
                                    ur: 'فیس وصول کریں'
                                },
                                'fees_tab_structure': {
                                    selector: '#fees-view .tab[data-tab="fee-structure"]',
                                    en: 'Fee Structure',
                                    ur: 'فیس کا ڈھانچہ'
                                },
                                'fees_tab_reports': {
                                    selector: '#fees-view .tab[data-tab="fee-reports"]',
                                    en: 'Fee Reports',
                                    ur: 'فیس رپورٹس'
                                },
                                'fees_tab_defaulters': {
                                    selector: '#fees-view .tab[data-tab="fee-defaulters"]',
                                    en: 'Fee Defaulters',
                                    ur: 'فیس نادہندگان'
                                },
                                'fees_search_student_label': {
                                    selector: 'label[for="fee-student-search"]',
                                    en: 'Search Student',
                                    ur: 'طالب علم تلاش کریں'
                                },
                                'fees_form_collect_title': {
                                    selector: '#fee-payment-form .form-title',
                                    en: 'Collect Fee',
                                    ur: 'فیس وصول کریں'
                                },
                                'fees_form_month': {
                                    selector: 'label[for="fee-month"]',
                                    en: 'مہینہ',
                                    ur: 'مہینہ'
                                },
                                'fees_form_year': {
                                    selector: 'label[for="fee-year"]',
                                    en: 'Year',
                                    ur: 'سال'
                                },
                                'fees_form_fine': {
                                    selector: 'label[for="fee-fine"]',
                                    en: 'Fine',
                                    ur: 'جرمانہ'
                                },
                                'fees_form_discount': {
                                    selector: 'label[for="fee-discount"]',
                                    en: 'Discount',
                                    ur: 'رعایت'
                                },
                                'fees_form_total': {
                                    selector: 'label[for="fee-total"]',
                                    en: 'Total Amount',
                                    ur: 'کل رقم'
                                },
                                'fees_form_payment_date': {
                                    selector: 'label[for="fee-payment-date"]',
                                    en: 'Payment Date',
                                    ur: 'ادائیگی کی تاریخ'
                                },
                                'fees_form_payment_method': {
                                    selector: 'label[for="fee-payment-method"]',
                                    en: 'Payment Method',
                                    ur: 'ادائیگی کا طریقہ'
                                },
                                'fees_form_collect_btn': {
                                    selector: '#fee-form .btn-primary',
                                    en: 'Collect Fee',
                                    ur: 'فیس وصول کریں'
                                },
                                'fees_form_receipt_btn': {
                                    selector: '#print-receipt',
                                    en: 'Print Receipt',
                                    ur: 'رسید پرنٹ کریں'
                                },
                                'fees_history_title': {
                                    selector: '#fee-history-container .form-title',
                                    en: 'Fee Collection History',
                                    ur: 'فیس وصولی کی تاریخ'
                                },
                                'idcards_title': {
                                    selector: '#id-cards-view > h2',
                                    en: 'ID Cards & Certificates',
                                    ur: 'شناختی کارڈ اور سرٹیفکیٹ'
                                },
                                'idcards_tab_generator': {
                                    selector: '#id-cards-view .tab[data-tab="id-card-generator"]',
                                    en: 'ID Card Generator',
                                    ur: 'شناختی کارڈ جنریٹر'
                                },
                                'idcards_tab_cert_generator': {
                                    selector: '#id-cards-view .tab[data-tab="certificate-generator"]',
                                    en: 'Certificate Generator',
                                    ur: 'سرٹیفکیٹ جنریٹر'
                                },
                                'idcards_form_title': {
                                    selector: '#id-card-generator .form-title',
                                    en: 'Generate ID Cards',
                                    ur: 'شناختی کارڈ بنائیں'
                                },
                                'idcards_member_type': {
                                    selector: 'label[for="id-card-member-type"]',
                                    en: 'Member Type',
                                    ur: 'رکن کی قسم'
                                },
                                'idcards_generate_btn': {
                                    selector: '#generate-id-cards-btn',
                                    en: 'Generate Cards',
                                    ur: 'کارڈ بنائیں'
                                },
                                'idcards_export_zip_btn': {
                                    selector: '#bulk-export-zip-btn',
                                    en: 'Export All to ZIP',
                                    ur: 'تمام زپ میں ایکسپورٹ کریں'
                                },
                                'cert_form_title': {
                                    selector: '#certificate-generator .form-title',
                                    en: 'Generate Certificate',
                                    ur: 'سرٹیفکیٹ بنائیں'
                                },
                                'cert_type': {
                                    selector: 'label[for="certificate-type"]',
                                    en: 'Certificate Type',
                                    ur: 'سرٹیفکیٹ کی قسم'
                                },
                                'cert_select_student': {
                                    selector: 'label[for="certificate-student-filter"]',
                                    en: 'Select Student',
                                    ur: 'طالب علم منتخب کریں'
                                },
                                'cert_achievement': {
                                    selector: 'label[for="achievement-description"]',
                                    en: 'Achievement Details',
                                    ur: 'کامیابی کی تفصیلات'
                                },
                                'cert_generate_btn': {
                                    selector: '#generate-certificate-btn',
                                    en: 'Generate Certificate',
                                    ur: 'سرٹیفکیٹ بنائیں'
                                },
                                'id_card_name_label': {
                                    selector: '.id-card-details b',
                                    en: 'Name:',
                                    ur: 'نام:',
                                    isGeneric: true,
                                    matchingText: 'Name:'
                                },
                                'id_card_father_label': {
                                    selector: '.id-card-details b',
                                    en: 'Father:',
                                    ur: 'والد:',
                                    isGeneric: true,
                                ...
                                'id_card_back_note': {
                                    selector: '.id-card-back p:first-of-type',
                                    en: 'This card is property of the institution. In case of loss, please return or notify at the address below.',
                                    ur: 'یہ کارڈ ادارے کی ملکیت ہے۔ گم ہونے کی صورت میں، براہ کرم نیچے دیے گئے پتے پر واپس کریں یا مطلع کریں۔'
                                },
                                'id_card_back_address': {
                                    selector: '.id-card-back p b',
                                    en: 'Address:',
                                    ur: 'پتہ:',
                                    isGeneric: true,
                                    matchingText: 'Address:'
                                },
                                'id_card_back_phone': {
                                    selector: '.id-card-back p b',
                                    en: 'Phone:',
                                ...
                                'settings_title': {
                                    selector: '#settings-view > h2',
                                    en: 'ترتیبات',
                                    ur: 'ترتیبات'
                                },
                                'settings_tab_general': {
                                    selector: '#settings-view .tab[data-tab="general-settings"]',
                                    en: 'General Settings',
                                    ur: 'عمومی ترتیبات'
                                },
                                'settings_tab_academic': {
                                    selector: '#settings-view .tab[data-tab="academic-settings"]',
                                    en: 'Academic Settings',
                                    ur: 'تعلیمی ترتیبات'
                                },
                                'settings_tab_theme': {
                                    selector: '#settings-view .tab[data-tab="theme-settings"]',
                                    en: 'Theme Settings',
                                    ur: 'تھیم کی ترتیبات'
                                },
                                'settings_tab_system': {
                                    selector: '#settings-view .tab[data-tab="system-settings"]',
                                    en: 'System Settings',
                                    ur: 'سسٹم کی ترتیبات'
                                },
                                'settings_general_title': {
                                    selector: '#general-settings .form-title',
                                    en: 'School Information',
                                    ur: 'اسکول کی معلومات'
                                },
                                'settings_school_name': {
                                    selector: 'label[for="school-name"]',
                                    en: 'School Name',
                                    ur: 'اسکول کا نام'
                                },
                                'settings_principal_name': {
                                    selector: 'label[for="principal-name"]',
                                    en: 'Principal Name',
                                    ur: 'پرنسپل کا نام'
                                },
                                'settings_academic_title': {
                                    selector: '#academic-settings .form-title',
                                    en: 'Academic Settings',
                                    ur: 'تعلیمی ترتیبات'
                                },
                                'settings_session': {
                                    selector: 'label[for="setting-current-session"]',
                                    en: 'Current Session',
                                    ur: 'موجودہ سیشن'
                                },
                                'settings_session_start': {
                                    selector: 'label[for="setting-session-start"]',
                                    en: 'Session Start Date',
                                    ur: 'سیشن کے آغاز کی تاریخ'
                                },
                                'settings_session_end': {
                                    selector: 'label[for="setting-session-end"]',
                                    en: 'Session End Date',
                                    ur: 'سیشن کے اختتام کی تاریخ'
                                },
                                'settings_theme_title': {
                                    selector: '#theme-settings .form-title',
                                    en: 'Theme Settings',
                                    ur: 'تھیم کی ترتیبات'
                                },
                                'settings_primary_color': {
                                    selector: 'label[for="setting-primary-color"]',
                                    en: 'Primary Color',
                                    ur: 'بنیادی رنگ'
                                },
                                'settings_secondary_color': {
                                    selector: 'label[for="setting-secondary-color"]',
                                    en: 'Secondary Color',
                                    ur: 'ثانوی رنگ'
                                },
                                'settings_reset_theme': {
                                    selector: '#reset-theme-btn',
                                    en: 'Reset Theme',
                                    ur: 'تھیم ری سیٹ کریں'
                                },
                                'settings_system_title': {
                                    selector: '#system-settings .form-title',
                                    en: 'System Settings',
                                    ur: 'سسٹم کی ترتیبات'
                                },
                                'settings_language': {
                                    selector: 'label[for="system-language"]',
                                    en: 'Language',
                                    ur: 'زبان'
                                }, */
            };
    ...
    <script>
        ...
        function sortTable(table, colIndex, direction) {
            ...
            const sortedRows = rows.sort((rowA, rowB) => {
                ...
                if (isNumeric) {
                    ...
                    return cellA.localeCompare(cellB, ['en', 'ur'], {
                        sensitivity: 'base'
                    }) * dirModifier;
                }
            });
        ...
        async function exportTableToPDF(table) {
            ...
            doc.autoTable({
                ...
                body: body,
                styles: {
                    font: window.amiriFontBase64 ? 'Amiri' : 'Helvetica',
                    fontSize: 8
                },
                headStyles: {
                    fillColor: [44, 62, 80],
            ...
            if (salaryStructure) {
                html += `<h4>Salary Structure</h4>
                 <div class="form-row">
                    ...
                    <div class="form-group"><label>Allowances:</label> <input class="form-control" value="${formatCurrency((parseFloat(salaryStructure.houseRent) || 0) + (parseFloat(salaryStructure.transport) || 0) + (parseFloat(salaryStructure.medical) || 0) + (parseFloat(salaryStructure.otherAllowances) || 0))}" readonly></div>
                ...
                html += `<h4>Salary Structure (From Profile)</h4>
                 <div class="form-row">
                    ...
                    <div class="form-group"><label>Net Salary:</label> <input class="form-control" value="${formatCurrency((parseFloat(teacher.basicSalary) || 0) + (parseFloat(teacher.allowances) || 0))}" readonly></div>
                 </div>`;
            }
    ...
    display: none;
        ...
        async function runAutomaticBackupCheck() {
            try {
                ...
                if (!settings || settings.autoBackupEnabled === 0) { // Check for boolean 0
        ...
        async function triggerAutomaticBackupDownload() {
            ...
            try {
                ...
                if (result.status === 'success') {
                    // This handles the download, as the PHP script also writes to a file.
                    // For automatic backups, you might just want it to save on the server without downloading
                    // For now, it will download, but you could comment out the download part if not needed.
                    const downloadLink = document.createElement('a');
                    downloadLink.href = `./backups/${result.filename}`;
                    downloadLink.download = result.filename;
                    //document.body.appendChild(downloadLink);
                    ...
                    localStorage.setItem('lastAutoBackupTimestamp', new Date().toISOString());
                    showNotification('Automatic backup created and saved on server.', 'success');
                    await cleanupOldBackups(appSettings); // Clean up after successful backup
                } else {
                    showNotification(result.message, 'error');
                }
            } catch (error) {
        ...
        async function generateTodaysAttendanceDetailsHTML() {
            ...
            for (const record of attendanceRecords) {
                ...
                if (student) {
                    const statusText = record.isLeave === 1 ? 'On Leave' : (record.status.charAt(0).toUpperCase() + record.status.slice(1));
                    const statusColor = record.status === 'present' ? '#2ecc71' : (record.isLeave === 1 ? '#f39c12' : '#e74c3c');
                    tableHTML += `<tr>
                            <td>${student.rollNumber || 'N/A'}</td>
                            <td>${student.name || 'N/A'}</td>
                            <td>${classMap.get(student.classId) || 'N/A'}</td>
                            <td style="color: ${statusColor}; font-weight: bold;">${statusText}</td>
                            <td>${record.isLate === 1 ? 'Yes' : 'No'}</td>
                          </tr>`;
                }
            }
        ...
        async function cleanupOldBackups(settings) {
            ...
            const allAutoBackups = (await getAllFromStore(STORES.BACKUP)).filter(b => b.isAutomatic === 1);
            let deletedCount = 0;
        ...
        function initializeShortcuts() {
            const shortcutConfig = {
                'alt+d': {
                    func: () => showView('dashboard'),
                    selector: '[data-view="dashboard"]',
                    title: 'Dashboard'
                },
                'alt+s': {
                    func: () => showView('students'),
                    selector: '[data-view="students"]',
                    title: 'Students'
                },
                'alt+t': {
                    func: () => showView('teachers'),
                    selector: '[data-view="teachers"]',
                    title: 'Teachers'
                },
                'alt+f': {
                    func: () => showView('fees'),
                    selector: '[data-view="fees"]',
                    title: 'Fees'
                },
                'alt+e': {
                    func: () => showView('exams'),
                    selector: '[data-view="exams"]',
                    title: 'Exams'
                },
                'alt+c': {
                    func: () => showView('classes'),
                    selector: '[data-view="classes"]',
                    title: 'Classes'
                },
                'alt+b': {
                    func: () => showView('backup'),
                    selector: '[data-view="backup"]',
                    title: 'Backup'
                },
                /* 'alt+i': { func: () => showView('id-cards'), selector: '[data-view="id-cards"]', title: 'ID Cards' }, */
                'alt+p': {
                    func: () => showView('salaries'),
                    selector: '[data-view="salaries"]',
                    title: 'Salaries (Pay)'
                },
                'alt+l': {
                    func: () => showView('library'),
                    selector: '[data-view="library"]',
                    title: 'Library'
                },
                'alt+r': {
                    func: () => showView('reports'),
